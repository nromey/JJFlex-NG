...
- **A** — Enter the audio layer — pick a target with one letter (`Ctrl+H` headphone, `P` PC output, `M` mic, `L` line out, `C` compander, `S` processor, `Ctrl+P` pan), ride Up and Down, place it at once with Home, End or `0`, Enter keeps it, Escape puts back everything you moved. Plain `H` lists the layer’s keys. The full walk is on the Keyboard Reference page
