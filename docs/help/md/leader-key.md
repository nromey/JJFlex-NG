...
- **A** — Enter the audio layer — pick a level with one letter (`V` slice volume, `Ctrl+H` headphone, `P` PC output, `M` mic, `L` line out, `C` compander, `S` processor, `Ctrl+P` pan) and ride Up and Down; flip a switch with one press (`Ctrl+M` mute, `Ctrl+A` PC audio on or off, `Ctrl+B` binaural). Enter keeps it, Escape puts back everything you moved. Plain `H` lists the layer’s keys. The full walk is on the Keyboard Reference page
