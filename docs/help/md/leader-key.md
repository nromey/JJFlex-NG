...
- **A** — Enter the audio layer — pick a level with one letter (`V` slice volume, `Ctrl+H` headphone, `O` PC output, `M` mic, `L` line out, `C` compander, `S` processor, `P` pan), ride Up and Down and place it at once with Home, End or `0`; flip a switch with one press (`Ctrl+M` mute, `Ctrl+P` PC audio on or off, `Ctrl+B` binaural). Enter keeps it, Escape puts back everything you moved. Plain `H` lists the layer’s keys. The full walk is on the Keyboard Reference page
