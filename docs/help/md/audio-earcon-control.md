...
- **Context help cue** — two quick rising taps a moment after you land on a control, meaning `Ctrl+F1` has something to say there. It is quiet by design: it only sounds when that help is different from the last help it announced, so tabbing through controls with no help, or controls sharing one explanation, stays silent. Turn it off here and nothing else changes.
