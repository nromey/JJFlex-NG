...
Under **Settings, then the Notifications tab, under Alert Sounds** you will find the master switch plus seven category switches, so you can quiet the sounds you do not want and keep the ones you rely on:
...
- **Context help cue** — a quick low flick and a soft note an octave above it, fading away, a moment after you land on a control, meaning `Ctrl+F1` has something to say there. It is the quietest sound in the app and it is meant to stay in the background: it is an offer, not an event. It only sounds when that help is different from the last help it announced, so tabbing through controls with no help, or controls sharing one explanation, stays silent. Turn it off here and nothing else changes.
