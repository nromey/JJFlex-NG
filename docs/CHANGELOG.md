...
- **[A second key reads the same signal in dBm](#dbm-on-a-key).** `Ctrl+J` then `Ctrl+S`. One S-unit covers 6 dB — a factor of four in power hiding inside a single reading — so when you want to know whether that antenna change actually helped, S-units aren't the tool. No mode to enter and no mode to remember leaving.
...
### A second key reads the same signal in dBm {#dbm-on-a-key}

Press `Ctrl+J` then `Ctrl+S` and JJ Flexible speaks your signal in dBm — "S meter minus 97 dBm" — instead of S-units.

Why bother, when `Ctrl+S` already reads the meter? Because an S-unit is a blunt instrument. One S-unit is 6 dB, which is a factor of four in power hiding inside a single number. If you're swapping antennas, comparing two stations, or trying to tell whether that new preamp did anything at all, "S7" both times tells you nothing while the signal quietly doubles. dBm tells you.

It matters a bit more after the fix above: the scale is trustworthy now, but it's still coarse. Coarse and honest is still coarse.

There was already a way to get dBm — a setting that switches `Ctrl+S` over to it permanently — and that setting is still there and still works. But a setting is a state you can be in the wrong one of. Flip it, get pulled into a QSO, come back an hour later, press `Ctrl+S`, and now you're squinting at a number trying to remember which units you asked for. Two keys means never having to know: `Ctrl+S` is always S-units, `Ctrl+J` `Ctrl+S` is always dBm, and you can take both readings of the same signal a second apart without changing anything.

The unit is always spoken, and the minus is a whole word rather than a hyphen, so there's no chance of "minus 97" arriving as "97."

One warning worth having in advance: in the JJ key layer, plain `S` toggles the radio's own spectral noise reduction and `Shift+S` toggles the one that runs on your computer. So the dBm key sits one finger-slip away from two noise-reduction switches. Both are toggles and both announce themselves by name, so if you land on the wrong one you'll hear it say so and a second press puts it back.
