...
- **You choose how mic-audio checks read** — plain English, decibels, or both — under Settings, Notifications, "Mic audio readout." Both is the default, which is exactly what you've been hearing all along.
...
- The Audio menu has a "PC Audio" group: PC Output Volume up and down (in dB — 12 is the loudness you've been living with all along, and you can now go quieter or a lot louder) and Mic Level. Changes apply the moment you make them, no reconnect, and your PC volume is remembered between sessions.
- The Audio menu's "On-Radio Outputs" group holds the radio's own jacks — headphone and line out volume, plus mutes for the headphone jack, line out, and front speaker — all plainly labeled "On-Radio" so you always know which side of the wire you're adjusting. Both groups are always there: plenty of people run PC audio and the shack speakers at once.
