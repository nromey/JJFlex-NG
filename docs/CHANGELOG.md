...
- **Starting up says less, and says it once.** A launch used to narrate itself four different ways before the radio list appeared: "Starting up, looking for radios on your network," then "Looking for radios on your network" a second time, then "Still looking for radios," and then the search window saying "Searching for radios" on top of all that. I'd already ruled that the whole thing should just say "searching for radios" and stop, and it turned out the ruling had reached the window and missed the speech. Now you hear "Searching for radios." once, "Still searching." if it's taking a while, and the window says its own name when it opens. That's it.
- **The main window announces itself once.** When a connect finished, and every time you Alt+Tabbed back, your screen reader read the window title, then Home, then "JJ Flexible Radio Access" a second time, then Home a second time. The second pair was your keyboard focus passing through an invisible container on its way to Home, and the container had a name, so it got read out. Focus goes straight to Home now, so you hear the title and your field and nothing else.
- **The radio count is corrected out loud when it changes.** The radio list tells you how many radios are online the moment it opens, and radios coming in over SmartLink can land a second after that. Until now the number you'd heard just stayed wrong; one day "1 online" and the next "2 online," depending on nothing you could learn from. Now when a radio turns up, the announcement carries the new total: "6300inshack is now available. 2 radios online."



- **The transmit and receive equalizers are back, and there's a band more of them than there used to be.** Don reported the graphic equalizers had gone missing, and he was right — though not for the reason any of us would have guessed. The equalizer window had been built months ago and then simply never connected to anything you could press, so it sat in there finished and unreachable the entire time. Nothing was taken away; it was never plugged in. It is now: **Receive Equalizer** lives under Noise Reduction and DSP, **Transmit Equalizer** under Transmission. Both give you nine bands where the old one gave you eight — 32 Hz has been sitting in your radio all along and we simply weren't showing it. Arrow a band up or down, or type the number you want and press Enter; either way it tells you where it landed. There's a switch at the top for the equalizer itself, because bands you can't hear are a cruel joke. **Restore** puts every band back the way you found it, **Clear** flattens the lot, and Escape leaves without keeping any of it — the radio goes back exactly as it was.
- **The app now knows when your screen reader has actually finished a sentence, so it stops repeating things you already heard.** Here is a bug I lived with for weeks without understanding it. You would disconnect from the radio, and a beat later the app would announce "Connected to FLEX-8600" — the very thing that had just stopped being true. Or you would press Tune and hear the tail end of the previous warning before the tune happened. The cause turned out to be simple and a little embarrassing: the app could hand your screen reader twelve seconds of speech in a single millisecond and had no idea which of it you had heard. So whenever a new announcement had to cut in, it assumed everything it had queued was lost and helpfully repeated it — including sentences you had already listened to in full. If you run NVDA, that is over. The app now hands your screen reader one sentence at a time and asks it what happened: spoken to the end, cut off at a certain word, or no answer. Something you heard is never re-spoken. Something that genuinely got cut off can still be rescued, exactly as before. And your own keystrokes no longer wipe out everything behind the sentence they interrupted — the rest still gets said. Under JAWS and other readers the app still has to estimate how long speech takes, and that estimate was about sixty percent too long, which is the direction that keeps stale announcements alive; it is now fitted to how a reader really speaks, and it learns your actual speaking rate over time. Nothing about this waits on your screen reader, so if it cannot answer, everything works the way it did yesterday. You should hear fewer repeats and fewer ghosts of announcements past. If you hear a repeat that should not have happened, or a sentence that should have been spoken and was not, tell me — every one of these is recorded now, so I can see exactly what your screen reader was told and what it said back.
- **The JJ key's help is a list you can read, not a speech you have to sit through.** Press `Ctrl+J` then `H` and, until now, the app recited every command in the layer in one breath — thirty-something of them, most of a minute, and if the one you wanted went by you started over from the top. I never liked it. It opens a list now: it tells you how many commands there are first, then one command per row, arrow through them at your own pace, type a letter to jump to that key, Escape when you're done. Short layers stay spoken — a handful of keys is a sentence, and a window would only get in the way. And from that list there's an **Explore the JJ key** button that opens something new: the whole JJ key as a tree. Plain letters, Shift letters, Control letters, Alt letters, help — and under a chord like `V`, the keys that only work once you're inside volume mode. Right arrow opens a branch, Left closes it, a letter jumps. It's built from the same table the help and the Command Finder read, so as the layers grow, the tree grows with them without anybody having to remember to update it.
- **You can copy what the app just said.** Don asked for this one, and it's the kind of request that makes you wonder why it wasn't always there. The app says something worth keeping — a signal report to drop in your log, an error you want to send to someone who can help — and now `Ctrl+J`, `Ctrl+C` puts it on the clipboard. It says "Copied," because a clipboard is invisible and a silent copy is indistinguishable from a dead key. It works with no radio connected too, which is deliberate: the moment you most want to paste an error message is usually right after the radio has dropped out from under you.
- **The speech history goes both ways now.** `Ctrl+F4` has always walked back through the last ten things the app said. It only ever went one direction, so overshooting the message you were after meant pressing your way all the way round again. `Ctrl+F5` now comes forward. Back on the left, forward on the right, next to each other — and `Ctrl+F4` still does exactly what it always did, so there's nothing to unlearn.
...
- **[Warnings can dip under the band, and it starts switched off now](#warning-dip).** When a warning sounds, radio audio through your computer can dip a few decibels so the alert lands on top of the band instead of inside it, then come right back. It shipped switched on. I have been living with it, and the dip turned out to draw more attention to itself than the warning it was making room for — a rare movement in your audio is more noticeable than a constant one, it turns out. **So it starts off now**, and if you were already using it and liked it, nothing changes for you: your setting is your setting, and this does not reach back and undo it. If you work a genuinely noisy band, or a warning has ever slipped straight past you, it is worth switching on — and there is more to it than there used to be. When I said the dip was distracting, what I actually went looking for was a hang time, and there was not one: the timing was welded shut and only the depth could move. Now, as well as a depth from barely-there to emphatic, the dip has a **pace**. **Quick** is how it shipped, snapping down and back with the sound. **Gentle** eases down and eases back. **Lingering** stays down a moment after the warning and comes back slowly, so the band breathes instead of stepping. My hunch is that a dip you notice more than the warning is one that moves too fast rather than one that goes too deep, and now that is something you and I can both test by ear instead of argue about. On every setting a deeper dip comes back more slowly than a shallow one, so the return never lurches, and nothing takes more than a second and a half to bring the band all the way home. All of it in one place: **Settings, Notifications tab, under Warnings and Radio Audio**. Worth knowing especially if you operate remote, where PC audio is the only audio you have got.
- **[Every sound has its own volume knob now](#per-sound-loudness).** If one alert reads louder to your ear than the rest — and ears differ, that's the whole point — you can trim just that one, up a little or down a lot, and it plays as you adjust so you're deciding by ear instead of by number.
...
- **[The layers listen to your verbosity setting now](#layers-honour-verbosity).** On Terse, opening the audio layer says "Audio layer." and stops — not the whole roll-call of letters — and a key the layer doesn't know gets the thunk and nothing more, because by then you know what the thunk means and where `H` is. Chatty keeps every word of the lesson. If your earcons are off, the short form is spoken instead, so a key you pressed never answers with silence.
...
- **[The keyboard comes back when another program takes it, and you can ask what's on your screen](#keyboard-comes-back).** Twice I sat in a healthy Select Radio dialog convinced the app had frozen; what had actually happened was that another program had taken the keyboard while I wasn't touching anything, and nothing noticed. Now the app notices, takes it back within a few seconds, and says so — and there's a checkbox in Settings, Accessibility if you'd rather it left your keyboard alone. And `Ctrl+J` then `Alt+W` opens a list of every window on your screen — the one with the keyboard first — so "what is in front of me?" is one chord away.
...
- **[Binaural has a control again](#binaural-has-a-control).** Binaural receive is the radio's own stereo widening — the same signal spread across your two ears instead of parked in the middle of your head. It has been in there working the whole time with nothing to press: Jim had a control for it, the control didn't survive the move to the new screens, and what came back was a keystroke you had to already know about, which isn't really a way in at all. There's a **Binaural** checkbox in Home's Audio and Slice group now (`Ctrl+Shift+U`), and a **Binaural** row on the Audio menu, and `Ctrl+J`, `V`, `Ctrl+B` still flips it from inside the audio layer. All three say "Binaural on" or "Binaural off" — the same words, whichever door you come through. It belongs to the radio rather than to one slice, so it changes every slice at once.
...
- **Pan gets its fine control.** The Slice field can slam a signal hard left, center, or hard right — three parking spots, nothing in between. But stereo placement is how you work two signals at once: A slightly left, B slightly right, and a pileup becomes two conversations instead of one mush. So now there's pan mode: press `Ctrl+J` then `Alt+P`, and Left and Right walk the slice you're on through the whole stereo field — hold Shift to move by one. Home or C snaps back to center, Escape reverts changes, out loud, for when you’ve nudged yourself somewhere you regret, and Enter accepts. It speaks the number — "Pan 40" — so you can write down an arrangement you like and dial it in again tomorrow. The old keys still do exactly what they've always done; this is the in-between they couldn't reach.
- **You can finally change your keys — for real this time.** Tools then Hotkey Editor opens the new Keys surface: every command in the app with its current key, its scope, and its default, arranged by scope, alphabetically, or by function group — plus a read-only view of all the built-in field keys. Pick a command, press Change Key, press the key you want — it works immediately, no restart. If the key already belongs to something else, JJ Flexible Radio names the collision out loud and asks before taking it. There's unbind, per-key reset to default, reset-all with a confirmation, and an Export Key List button that writes your whole key map to a file you can read or share. The old Key Assignments window — the one where Update looked like it saved and never actually did — is gone. Help then Key Assignments opens the same surface for browsing, one menu item instead of three.
- **Press Shift slash on any Home field and it speaks that field's keys.** "Keys on Frequency: Up Down coarse tune..." — the field you're on first, then the keys that work everywhere in Home. Same wording as the per-field help dialog and the Command Finder, because they all read from one table now, so they can never drift apart again.
- **Your tuning step sizes have keys of their own now.** Modern tuning is the mode built entirely around a coarse step and a fine step, and until now the only way to change either one was to stop what you were doing and open Settings. Ctrl+F1 on the frequency field would even read you both numbers and offer Shift+S to hear them again — which is a particular kind of insult, naming a number, letting you re-hear it, and giving you nothing to do about it. There's one rule now and it covers the whole field: **up and down tune, left and right size, and Shift makes it fine instead of coarse.** Alt+Left and Alt+Right make your coarse step smaller or larger; Shift+Left and Shift+Right do the same for your fine step; Left is always smaller and Right is always larger. The sizes walk a short list of the ones people actually reach for — 500 Hz, 1, 2, 5 and 10 kHz for coarse, and 1, 5, 10, 50 and 100 Hz for fine — and that list deliberately doesn't wrap round: at either end you'll hear the size followed by "smallest" or "largest", instead of one keypress dumping you at the far end in the middle of a QSO. Press S on the frequency field to see the whole list and set both at once. Settings → Tuning offers exactly the same values now, because it's the same list. Alt is on the coarse pair rather than Shift for a plain reason: bare Left and Right move your cursor across the Home fields, in both tuning modes, and that isn't going anywhere.
- **Heads-up on Shift+Left and Shift+Right.** They used to move your cursor across the Home fields, exactly like Left and Right on their own — the Shift may as well not have been there. On the frequency field in Modern tuning they now size your fine step. Everywhere else in Home they still move the cursor, unchanged.
- **One honest note about step sizes, old and new:** they don't yet survive closing the app. That has always been true of the Settings → Tuning values, and it's true of the new keys too — you'll start each session back at your coarse 5 kHz and fine 100 Hz. Making them stick is a separate piece of work and it's on the list.
...
- **No key in Home plays dead any more.** Press a letter that isn't bound on the field you're on and JJ Flexible Radio says so — "S does nothing on the Volume field" — and points you at Shift slash, which speaks the keys that do work right there. With no radio connected, the Home keys answer "No radio connected" instead of nothing at all. A silent key is indistinguishable from a broken one, and you should never be left guessing which you've got.
- **Shift and Ctrl mean something in Home now.** On the Slice field, `A`, `Shift+A` and `Ctrl+A` all jumped to slice A — Home was reading the letter and ignoring whatever else your hand was holding, and it did the same with every letter and digit on every Home field, so `Ctrl+1` on the frequency field typed a 1. A plain letter still does exactly what it always did. A letter with Shift or Ctrl held now does only what it's actually bound to, and if that's nothing, Home tells you so in the same words it uses for a plain letter: "Control A does nothing on the Slice field." Heads-up if your fingers had learned the accident: `Shift+A` through `Shift+H` on the Slice and Slice Operations fields no longer pick a slice. The plain letter does that, and inside the JJ key, Shift plus the letter still jumps to that slice from anywhere.
- **Heads-up: split moved to P on the frequency field — and it toggles now.** S has been the split key there since the Jim days, but it only ever turned split *on*, and in Modern tuning S belongs to the step sizes — the same letter doing two different jobs depending on your tuning mode. So split gets one letter that works the same everywhere: P turns split on *and* off from the frequency field, in Classic and Modern alike — and on the Split field itself, right beside Space. One letter, everywhere split lives.
- **Heads-up: the S "sound" key on Slice Operations is gone.** It unmuted a slice that, most of the time, was already unmuted — so it announced "slice A sounding," played a tone, and changed nothing, which made it read like a broken key. M still mutes outright from that field, and Space still toggles, which covers unmuting. One less key to wonder about.
- **Switching tuning modes no longer teaches you a key that doesn't exist.** The Modern tuning announcement had been saying "Press C to switch to fine tuning" — and C was retired versions ago, so it did nothing at all, at the exact moment you were trying to learn the mode. The announcement now gives you the mode's name, and at chatty verbosity points you to Ctrl+F1 on the frequency field, which reads the real, current key map — the same one the keyboard reference shows.
...
- **[The S-meter is honest now — and your readings will change](#smeter-tells-the-truth).** Every S-unit this app has ever spoken was about half an S-unit shy of the international standard, and on 6 meters and up it was three to four whole S-units shy. Both are fixed, so signals now read a little stronger than you're used to. Please read this one before you file a bug.
- **[The S-meter's units are yours to pick, and they stay picked](#smeter-units).** `Ctrl+J` then `Ctrl+S` switches between S-units and dBm, tells you which one you landed on, and remembers it for that radio — so `Ctrl+S` reads the meter in whichever unit you chose, this evening and next month. One S-unit covers 6 dB, a factor of four in power hiding inside a single reading, so dBm is the one to be in when you're asking whether that antenna change actually helped. It's on the Radios tab in Settings too, if you'd rather see where it stands than press a key to find out.
- **The QSO signal analyzer — hear what a signal actually did.** "Is he fading? Is that QSB? Is he coming up?" You can't answer that from meter readings spoken one at a time, and the speech would talk over the very signal you're straining to hear. So now there's a stopwatch for signals: press `Ctrl+J` then `Ctrl+Q` when a station starts talking, work the contact in silence, and press it again when you want the answer. "Peaked S 9, fell to S 4, averaged S 6. Deep fades about every 5 seconds." That's an honest signal report with evidence behind it — into your log or straight back on the air. Every capture saves itself under Tools, then Signal captures, where the full report lives: peak, trough, average, fade rhythm and depth, trend, and exactly how each number was measured. You can rename a capture there too — "Don on 40 meters" beats "the one from 9:14" a week later — plus export it to send to a friend, or delete it. It runs until you stop it, and the what's-on machinery has its back: `Ctrl+J` then `O` reports it, it pipes up on its own if you forget it past fifteen minutes, and closing the app asks about it on the way out. Your own transmissions are left out of the numbers automatically, and anything a capture couldn't determine says so in as many words — a ten-second capture doesn't guess at a fade rhythm, and "no data" never sounds like "a quiet band." This one's for Don, whose question it answers.
- **[Switching screen readers mid-session works now](#reader-swap).** Start JJ Flexible Radio under one screen reader, switch to another, and the app kept talking to the one that left — so the new one had nothing to say and fell back to reading the screen itself. It notices the swap within a few seconds now, moves your speech over, and the new reader says "Speech now through JAWS" in its own voice so you know it landed.
- **[Push to talk stays keyed while you hold it](#ptt-stays-keyed).** Under some screen readers a single held Ctrl+Space was keying and unkeying the radio three times before it settled — real transmit, switching on and off, while you were mid-sentence. That's fixed, and the fix reads your own Windows keyboard settings instead of a number somebody measured once on one machine.
...
- **[Safety check on port forwarding](#port-forwarding-safety-check).** If you're connected to somebody else's radio via SmartLink and try to change the port settings, JJ Flexible Radio politely stops you instead of overwriting their radio's config.
- **[Each radio now remembers its own connection settings](#radios-tab)** in the new Radios tab under Settings — Tools then Configure Radio jumps straight there. One radio can use a forwarded port while another hole-punches, and it's all editable whether you're connected or not. Oh, and you can finally rename your radio from inside JJ Flexible Radio instead of borrowing SmartSDR for it.
...
- **Settings that have to wait now say so out loud.** Some things can't take effect until you're connected — a name for a radio that's off, or its remote power setting. Press OK and JJ Flexible Radio lists what's waiting in a small dialog you can arrow through and re-read at your own pace, instead of letting you assume it all happened.
...
- **You can now tell JJ Flexible Radio "I can't get to this radio."** A per-radio setting for rigs that live somewhere you can't walk to — a remote site, the far end of a SmartLink link, a friend's shack across town. Checking it offers to set up the safety net such a radio needs — REM ON, the remote administration allowances — with every setting named and explained before anything changes, and a receipt afterwards that you can re-read. There's a "don't show this explanation again" for the snowbirds who flip it twice a year, but the receipt always shows: nothing changes on your radio behind your back.
- **"Change nothing on this radio" — a hold for the days a radio must stay exactly as it is.** Connecting has always done a little tidying on the way in — loading your profiles, straightening a few settings. Great on your own rig; exactly wrong on a radio that was just factory-reset for testing, or a friend's station you're only borrowing. So each radio now has a checkbox on the Radios tab: change nothing on this radio. Tick it before you connect — no radio needed, that's the point — and JJ Flexible Radio connects, listens, and operates as usual, but leaves the radio's own setup alone. No profiles created or loaded, nothing applied at connect, no changes to its name, network, registration, or firmware. And it's never silent about it: you hear that it's on at each connect, and anything declined because of it says so and names the setting — so a setting that won't stick is never a mystery. Off unless you turn it on, and every radio behaves exactly as before until you do.
...
- **[SmartLink sign-in is a real dialog now](#native-signin)** — email box, password box, Sign In button, Forgot Password button. No web page, no browser, no fighting a login form that won't talk to your screen reader. And signing in this way sticks: JJ Flexible Radio quietly renews it in the background from then on.
...
- **Connect tries the other door before giving up.** If your radio doesn't answer the way it usually does — the network hiccuped, the radio moved, whatever — JJ Flexible Radio now tries the other path and tells you it's doing it, instead of stopping at the first no. Nothing happens in secret: every switch is announced, so you always know whether you're on your own network or going through SmartLink. Radios you take places — Field Day, a club shack, a new house — just keep connecting.
...
- **Connecting says less, and all of it is true now.** A connect used to talk over itself: "no active slice" from one corner of the app while another corner was still waiting for your slices to arrive — a sentence that was false within two seconds — plus "JJ Flexible Home" flickering at you as focus got dragged home between the connect windows. Now nothing claims your slices are missing while they're still on their way, and focus stays with the connect until it's actually done. The full arrival rundown — "Connected to FLEX-8600, 4 slices, Slice A..." — is still the big finish, and now it isn't fighting four other sentences to reach you.
- **The connect windows hand off to each other now, so File Explorer stays out of it.** This one I had to measure to believe. A connect walks you through three little windows — searching for radios, pick one, connecting — and each of them used to close a hair before the next one opened. In that hair, your screen reader would start reading out whatever window was behind them. On my machine that was File Explorer, and I heard "win-x64" at the start of every connect for weeks without ever working out why. The next window is now up before the previous one goes, and when the connecting window finishes, it hands your focus straight to the main window itself rather than trusting Windows to guess where it should land — a guess that was landing on File Explorer on a fresh launch even after the rest was fixed. While that window waits for the radio list to appear, its title now says what's coming rather than what's finished. And "Connecting" itself is spoken only when the connect is actually taking a moment. On a quick home-network connect the word was getting cut off partway anyway, and you'd hear it one time and not the next for no reason you could learn from. Now a fast connect stays quiet and lets "Connected to..." do the talking; a slow one — SmartLink, say — tells you what it's connecting to and which way it's going, once it's clear you're in for a wait.
- **Your slice count arrives once now, not twice in two dialects.** The connect used to say "4 slices out of 4 used, slice A USB" and then, a tenth of a second later, "4 slices. Slice A..." inside the full welcome — the same fact, back to back, worded two ways. The full welcome owns it now. On a slow connect, where the welcome goes out before your slices have landed, the count still speaks on its own the moment they settle — you never lose the information, only the echo. The Morse census is untouched; that channel answers to its own switches.
- **The connect double-beep sounds on every connect now — and it was covering for two silent friends.** The little arrival beeps went missing, and the hunt turned up something bigger than a sound effect: the app was skipping the whole "you've just connected" moment. That moment is also where your radio-audio-through-the-computer choice gets applied, and where a REM ON setting you made while the radio was off finally reaches it. All three happen on every connect now. The beep was the only one of the three you could *hear* go missing — which is exactly why a missing beep was worth chasing.
- **The connect beeps climb now, so arriving sounds nothing like still working.** Here's a good one. The "you're connected" beeps and the "still connecting" beeps were the same two beeps, at the same pitch, the same length, the same everything — the arrival pair was simply a bit louder. Nobody could have caught it, because the arrival beeps hadn't actually sounded once since the day they were written (that's the fix just above), so the very first time they played was also the first time anyone could hear the problem. And loudness on its own is a rotten way to tell two sounds apart, since there's nothing to hold it up against in the moment. So the connect sounds are a ladder now. The beeps still count — one, two, three — to tell you how far along a connect has got, because counting was the right idea and it stays. What's new is that each step sits higher than the one before it, and the connected pair sits highest of all: a full octave above the beeps you'll usually hear right before it. Low means starting, high means you're in. On a quick home-network connect that's two quiet beeps and then two higher, louder ones — a rise, not a repeat. If you'd like to hear the whole set back to back without connecting anything, Help then Earcon Explorer has a Connection group with a "play all in order" button, and each one tells you its pitch. And after a day of actually listening to them, those beeps are also **a bit over twice as long** as they were. That sounds like a fussy detail and it really isn't: when I went looking at why some of my alert sounds vanish under a busy band, length turned out to matter more than pitch or volume. Below about a twentieth of a second your ear files a beep as a click and throws the pitch away, and the sounds that survived a crowded band were the longer ones. The connect beeps were sitting on the wrong side of that line, which is exactly why they'd disappear on you right when you most want to know whether you're still connecting. They're at the short end of the range that survives now. I stopped there on purpose rather than going longer still — stretch them much further and two steps of a slow connect start to tread on each other, and a count you can't count isn't worth the extra reach. The Explorer has a row that plays the old short version if you want to hear what changed.
- **Disconnect answers you right away now, and gets the last word.** Pressing Disconnect used to buy you two full seconds of nothing — long enough to wonder if the key was dead — then a sentence about Home, and only *then* the actual news, four seconds late. Now you hear "Disconnecting from your radio" the moment you press it, Home arrives while the app winds things down, and "Disconnected" lands at the end, once, when it's true. Behind the scenes, three different corners of the app were each announcing the same disconnect and cancelling each other so thoroughly that nothing survived — it's one voice now.
- **Closing a dialog with no radio now tells you where you landed.** Leave Settings without a radio connected and your screen reader used to say just "Pane" — the sound of a room nobody named. Now you land back on Home and hear so: "JJ Flexible Home, no radio connected." Works however you leave the dialog — OK, Cancel, or Escape.
...
- **Busy radio, clean exit.** Connect to a radio whose slices are all taken — somebody's already operating on every one of them — and JJ Flexible Radio now tells you who has them and backs out on its own. It used to announce the problem and then sit at "Connecting" indefinitely, quietly waiting for you to dismiss an error window you had no way of knowing existed.
- **You stay recognized at your own radio.** Mid-connection, a Flex sometimes drops and re-adds the list entry for your own connection with the identity fields blanked — and JJ Flexible Radio would take that at face value: suddenly "another client connected" out of nowhere, and worse, the app could decide you were no longer the operator at the radio and refuse to let you change network settings you were sitting right in front of. It keeps its own memory of who you are now and doesn't fall for it.
...
- **Your default account is actually in charge now.** With more than one SmartLink account saved, JJ Flexible Radio used to quietly favor whichever account it had touched most recently — and "touched" included its own background housekeeping, so the wrong account could win forever no matter what you set as default. That's how a few of you saw a registration reminder naming somebody else's account on your own radio. One rule everywhere now: your default (or the account you picked with Use Now) is the account in play, full stop. And when several accounts are saved with no default chosen, JJ Flexible Radio asks you to pick one instead of guessing — it will never quietly sign in as someone else again. The account manager also lists your default first now instead of whoever was used last.
- **The radio list knows whose radio is whose.** An offline radio that belongs to a different SmartLink account now says so right on its row — "registered to don@example.com, last seen 2 days ago" — instead of reading like an anonymous remote radio. Press Enter on it and JJ Flexible Radio switches to that account and refreshes the list for you, announced first, instead of grinding through a half-minute search on an account that could never find it. And every SmartLink connect now speaks which account it's using before it starts — for every radio, every time, so there are no surprises about whose account is on the air. The offline rows also say "last seen" exactly once now, which they did not always manage before.
- **The radio list tells you when somebody's already on a radio.** A radio with another station connected now says so right on its row, before you connect — "one other client, k5ner" — with the station's name when it gives one. So if you're about to borrow a friend's rig, you'll know whether the owner is sitting on it before you ever key up. A radio with nobody on it says nothing extra at all: most rows are empty most of the time, and your screen reader doesn't need to announce the obvious on every arrow press.
- **A radio can have its own account.** If you share a club rig — two operators, two SmartLink accounts, one radio — you can now tell JJ Flexible Radio which account that radio should always use: press the Applications key (or Shift+F10) on the radio and pick Preferred Account. There's also an Associations button in the SmartLink account manager that shows every radio this computer knows and which account each one uses, and lets you change the binding from the account side. Radios without a preference keep working exactly as before — automatic, no setup needed.
...
- **The [Connecting screen now lets you out](#connecting-cancellation).** Press Escape or click the X close button and JJ Flexible Radio cancels the connection attempt right away — no more waiting for a timeout, no more force-quitting the app. The screen also tells you what phase it's in as it works, so you know whether SmartLink is slow, the slice is still being acquired, or something else is up. If a connect takes more than a minute, you get a "keep waiting or cancel" prompt with the actual reason; at five minutes JJ Flexible Radio cancels for you so the screen never sits there spinning forever.
...
- **[The installer brings everything it needs](#self-contained).** Run the new Setup on a fresh Windows machine and JJ Flexible Radio just installs. No more "you need .NET 10, click here to download Microsoft's runtime first." The download is about 55 MB instead of 10 — bigger, but it's one trip instead of three, and that "go install this other thing" detour was an accessibility wall for screen reader users on new machines. Done.
...
- **The receive check tells you whether audio is actually arriving now.** Everything it used to report was a setting — this output is muted, that level is at 60, radio audio through your computer is on. All true, all worth knowing, and every bit of it something *we* set. None of it could tell you whether a single scrap of sound had ever made it from the radio to your computer, which is the first thing Flex would ask about. So the check now watches what the radio is actually sending and reports what it saw: “Audio arriving from the radio: up to 120 kilobits per second, in every one of 30 readings taken about a second apart, counted from the first reading that carried audio.” And it knows the difference between audio that hasn’t started yet and audio that’s cutting out. The watcher starts the moment you connect, a beat before the radio actually starts streaming, so on a first run the earliest readings are blank while everything warms up — and those blanks used to be counted, which made a perfectly healthy radio read short on exactly the run most people take. Not any more: the tally starts when the audio does, and the report says so in as many words. Blanks *after* the audio has begun are another matter entirely — that’s sound going missing mid-stream, which is what a weak or congested connection looks like — so those stay in, counted and called what they are. If you’re on SmartLink wondering whether the chop is your radio or your network, that’s the line that answers you. Each figure says why it’s worth reading, too: meters still arriving while audio is not means the radio is talking to your computer but not sending sound, which is a different problem from a dead link, and the report says that right where the number sits. And when everything checks out, the answer names its subject — “Nothing obvious is wrong with your receive audio” — so a pasted report still makes sense when it lands in somebody else’s inbox. If nothing is arriving while radio audio through your computer is switched on, it says so — and tells you whether anything *else* is arriving either, because “the radio is sending its meters but no sound” and “nothing at all is reaching this computer” are two completely different problems with two completely different fixes. And if you listen on the radio’s own speaker, where no sound is meant to come across the network at all, it says none is expected instead of crying wolf at the most ordinary setup there is. Don asked for this one.
...
- **[A failed connect tells you why](#connect-failure-honesty).** No more bare "Connection failed." You hear what the evidence says — the router refused, the packets vanished, the sign-in was rejected, the radio never showed up in your account — and when the router rule is the problem, JJ Flexible Radio reads you the exact rule your router needs, built from what the radio itself reports.
...
- **[VOX has its gain and delay back, and the delay was never telling you the truth](#vox-settings-return).** Don pointed out that VOX could be switched on but not set up — no gain, no delay, nothing to adjust — and that it didn't work, which follows. The two numbers are in the Transmission group of Home right after the VOX checkbox, and in the audio layer on `X` and `D`. While putting them back I found the delay had been scaled wrong since the very first version: what the app called two seconds was really 800 milliseconds, and the top sixty percent of the radio's hang-time range was out of reach. It's right now, and there's a test standing on it.
- **[The tracking notch filter is reachable again](#tracking-notch-returns).** `Ctrl+J` then `Alt+N`, or the DSP section of Home. Your radio has been ready to place tracking notches at every connect for as long as I can remember; the window that let you place one was deleted during the rewrite and its replacement was never hooked up to anything. Auto Notch hunts a carrier for you — this is the one where you point at the birdie yourself, set how wide and how deep, and tell the radio to hold it through band changes and power cycles.
...
- **Ctrl+F1 on Home finally answers — and it knows which tuning mode you're in.** I designed the two tuning modes and I still forget their keys, so this one is as much for me as for you. Press Ctrl+F1 on the Frequency field and JJ Flexible Radio tells you which mode is live right now, then reads only that mode's keys: in Classic it names the digit your cursor is sitting on (the one Up and Down actually tune by), in Modern it speaks your real coarse and fine step values, and either way it ends by naming the key that switches to the other mode. Every other Home field answers Ctrl+F1 too, with the same key list the ? key speaks. It all reads from the same table as the keyboard reference, so the spoken help can never quietly drift out of date again.
- **"No extra help here" now leaves you a move.** When a control truly has nothing extra, Ctrl+F1 tells you where help does live: F1 for the help file, Ctrl+slash to search every command.
- **Tab in Settings goes into the settings now, not out of the dialog.** Settings opens on the category list, which is how you choose what you want to change — so Tab from there ought to take you to the thing you just chose. It was taking you to OK, Apply and Cancel instead, which is the one place you definitely weren't headed. Category first, then that category's settings, then OK, Apply and Cancel last, where buttons that end a dialog belong. Ctrl+Tab and Ctrl+Shift+Tab still hop between categories from anywhere inside, and Shift+Tab from the first setting puts you back on the category you came from.
- **Escape stops a transmit check now, even with the carrier up.** The transmit checks under Tools, then Fix key your radio for real — a couple of seconds on the transmitter step, and the better part of half a minute on the tones-and-voice one. Pressing Escape or Stop while that carrier was up did exactly nothing, because the app was busy taking the measurement and couldn't hear you until it had finished. That's the wrong way round: while you're on the air the dangerous thing is the wait, not the stop. Escape now goes straight to the transmitter no matter how busy the window is. The carrier drops in a fraction of a second, you get an alert tone the moment the press lands, and then a plain sentence telling you which of two things happened — "Transmit stopped," or, if the radio didn't do as it was told, that it still says it's transmitting and to switch it off at the front panel. A stop that failed will never sound like a stop that worked. And while a check is keyed, your reflected power is being watched the whole time now, which — despite appearances — it genuinely wasn't before.
- **If most of your power is coming back at you, JJ Flexible Radio now ends the transmission — and there's a switch if you'd rather it didn't.** Here's the scenario this was built for, because it happened on my own bench: transmitting into an empty antenna jack while the dummy load sat on the other port, 76 percent of the power coming straight back at the finals, and the SWR meter swearing everything was fine. Now, once the reflected-power warning has fired and you're still keyed above 10 watts, your transmission gets ended and you're told plainly what happened, why, and that you are no longer on the air. Your radio already protects itself by folding power back and sounding an alarm — this is a third layer on top, for the operator who can't glance at a meter mid-sentence. And because some of you run experimental antennas, reactive loads, or an outboard tuner you adjust on the air, it's a setting, not a decree: one checkbox on the PTT tab of Settings, on by default. Turn it off and the warning itself will remind you, every time it fires, that no cut is coming — a safety you switched off should never be a safety you still think you have. Neither the warning nor the cut will ever interrupt your antenna tuner mid-cycle; high reflected power during a tune is the tuner doing its job.
- **The reflected-power cutoff now watches the watts coming back, not just the share.** Until now the cutoff only ever acted after the reflected-power warning had fired, and that warning listens for a *share* — forty percent or more of your power coming back. Which left a hole I should have seen sooner: run a hundred watts into a so-so match and eighteen watts can be coming straight back at your finals while the share sits at eighteen percent, well under anything the warning cares about. Watts are what heat a transmitter, not percentages. So there's a second trigger now, and it doesn't wait for the warning: ten watts or more coming back on two readings in a row, and the transmission is ended. You'll hear it in watts — "13 watts of your power were coming back on ANT1" — so you always know which measurement acted, and the share-based cut still says percent, because a percentage is what tells you how bad the *match* is. Same switch on the PTT tab of Settings covers both; turn it off and both stand down together. I'll be honest about the number: ten watts is a first figure, chosen from reasoning about heat rather than from a measurement, and the empty-port bench test that would confirm or move it hasn't been run yet. And a related fix to the SWR you're shown: it used to be worked out on readings as low as a twentieth of a watt, which is deep in the noise, and the safety watch and the display disagreed with each other about where the noise ended. There is one floor now, and below a watt of forward power you'll hear "not measured" rather than a number nobody should trust.
- **The reflected-power alarm now waits for a tuner to finish hunting.** One of my testers runs a remote tuner off a radio with no internal one. He keys up, the tuner out at the antenna starts searching, and for a few seconds most of his power genuinely is coming back while it looks for the match. The alarm saw a real number, did exactly what it was built to do, and ended his transmission about a second before his tuner settled to 1.7. The radio wasn't lying and neither was the alarm — it just couldn't tell a fault from a search. Now it can, and you don't have to tell it a thing about your tuner: a bad antenna's reflected power holds still, while a tuner hunting produces reflected power that jumps around and falls. So if the reading is high but still moving, the alarm holds off, for up to twenty seconds. If it settles back to a good match, nothing is said at all. If it settles high, you hear the warning then. And if it's still bouncing around after twenty seconds, that isn't a tune any more and you hear it anyway. Holding off is not switching off: a tuner that never finds a match is precisely the moment you need telling. One more thing while I was in there — if you bring up the tune carrier (Ctrl+Shift+T) while you're keyed, to drive an outboard tuner, the alarm stands down for as long as that carrier is up, and it's the radio itself that says when the carrier is down, so there's nothing to get stuck.
- **The transmit checks carry you forward now, and you still hear every result.** Every button on that page rebuilds it, and that part is honest — what the page says genuinely changes when you press something. What wasn't honest was leaving you at the top afterwards, so every press cost you the press plus the walk back down to where you were. For a whole run. Then I fixed that by landing you on the check you'd just dealt with — better, but still a reading trip past things you'd already handled to reach the thing you hadn't. So here's the rule the page follows now: what happened is spoken, and you land on whatever comes next. Answer "can you hear the radio right now?" and you hear your answer read back while landing on that check's Run button. Run a check and you hear how it went — the same words its heading shows — while landing on the button for what's next: the one-press fix if it found something fixable, "Next" if it passed or you skipped it, or back on the Run button to try again if the check couldn't run at all. Press a fix and you hear what the setting actually became while landing on "Run this check again," which is how you prove the fix took. Change the power and you come back to the Run button, which now tells you the new wattage it would use. You can walk an entire run top to bottom without once navigating backwards, which is what a sequence of checks should have felt like all along.
- **A check you just ran no longer sounds like one you didn't.** When a check passed, the page used to helpfully move you along to the next one. Which meant the first thing you heard after running the audio setup check was the *next* check announcing itself as "not yet run" — and the result you pressed the button for went past unheard. Sitting at the radio, that sounds exactly like being told you hadn't run it at all, and I spent a while convinced that's what it was saying. The result is spoken the moment the check finishes now — pass, problems, or couldn't run — so what you pressed the button for is the thing you hear.
- **The emergency control is called "Stop everything" now.** The button at the top of the checks page used to say just "Stop," and I read it myself as "stop the test run" — on a calm evening, with no RF in the air. It's not that button. It's the one that halts whatever is happening right this second, keyed transmitter included, dropping the carrier first and asking questions after. A control you reach for while something's wrong has to be recognisable while something's wrong, and "Stop everything" says what it does. Ending the tests themselves is a different, calmer decision — read on.
- **Leaving the checks now offers the three choices that actually exist.** Escaping mid-run used to ask a yes-or-no that warned exiting would cancel your test — while the run was, in fact, already safe on disk, because test runs save themselves as they go. A warning that frightens you out of closing a window with something that isn't true is not how this tool is supposed to work. Now the way out asks what you actually want: continue the test, which is the safe default; exit without saving, which really does delete the saved run and tells you what you'd be throwing away — measurements that keyed the radio cost real transmission; or stop the tests and resume later from View or resume saved test runs on the Fix menu, which the app could always do and simply never offered at the moment you'd want it. Escape and the window's close box ask the same question, so there's no wrong door.
- **The checks page opens with how to use it — including how to get out.** A short expandable section now sits at the top: how to move around, that the flow carries you forward, every way to leave, and what Stop everything actually does. It's open the very first time you run the checks and folds itself away once you've saved a run; open or close it yourself and it stays how you put it for the rest of the session. I asked "is Escape the only way to exit?" about my own tool this week, which told me everything I needed to know about how discoverable the exits were.
- **The report now points at your radio's manufacturer, not at Flex by name.** The sentence beside the Copy button says the email-ready report goes to your radio's manufacturer, and the try-it-in-the-manufacturer's-own-software advice names SmartSDR as the Flex example instead of assuming everyone owns it. Today every radio JJ Flexible Radio speaks to is a Flex, so nothing changes for you — this is the wording getting honest ahead of the day it wouldn't have been.
- **A check that refuses now says so out loud.** "Something is already running," a fix that didn't take, a stage that won't be skipped because it already has a measurement worth keeping — all of that was being written onto the page where you'd see it if you could see it. It's spoken now. On the one screen in this application whose entire job is finding things that fail quietly, that was a bit rich.
- **Your radio gets judged by your account, not the one you borrowed.** Connect to a friend's rig over SmartLink, disconnect, then connect to your own radio sitting three feet away — and JJ Flexible Radio would greet you with a note that your radio isn't registered with SmartLink under *their* email address. Every word of that was true and none of it was any of your business: it's the answer to a question nobody asked. When you borrow somebody's account to reach their radio, that borrowing now lasts exactly as long as the connection you borrowed it for, and disconnecting hands it back. And if a local connection somehow still gets weighed against an account you didn't pick as yours, JJ Flexible Radio now says nothing at all rather than raise the question. That matters more than a confusing box, because that box has an "I only use this radio here" button that settles the matter permanently, for that radio — and it was putting it in front of the wrong one.
- **The radio list stops calling a radio missing when it never went looking.** A radio that lives on somebody else's SmartLink account can only be seen while JJ Flexible Radio is signed in to that account. The list didn't make that distinction: the row said "not heard from" whether anyone had asked or not, so Don's 6300 read as missing about ten seconds before I connected straight to it. That row now tells you the truth — "not checked, not signed in to" and then the account it belongs to — so a quiet radio and a door nobody knocked on don't sound the same any more. Press Enter on it and it still does what it always did: switches to that account, goes and looks, and connects.
- **A button says its name now, not its life story.** Some buttons showed three words on screen and then read you a whole sentence — every single time you landed on them. "Release All Extra Slices" was announcing "release every slice except the one you are on, back to one slice." That's true, and it's genuinely useful exactly once; by the four hundredth landing it's just something standing between you and the next control. Twelve buttons and checkboxes now say what's written on them, and the sentence each one used to recite moved to `Ctrl+F1` — press that on any control and JJ Flexible Radio explains it, which is what `Ctrl+F1` has been for all along. The worst of them lived in the SmartLink accounts window, where one button read out twenty-five words before you could get past it while five of its neighbours sat there behaving perfectly. There are more of these and I have a list; the app checks itself for new ones from here on, so at least the pile stops growing.
- **A reference recording ships in the box.** It's the identical audio on every station running JJ Flexible Radio, so a reading you take means the same thing as a reading I take. On it: a slate, a level tone, a steady passage long enough for the loudness meters to settle, the phonetic alphabet, counting, the same sentence said quietly then normally then loudly, a run of plosives and sibilants to hear what your processing does to them, and a few seconds of silence for the noise floor. The script it reads sits right beside it as a plain text file, so you can find out what you're about to transmit without transmitting it first. Read that same script into the recorder and you've got a reference in your own voice, on your own microphone, in your own room — the honest one for your station — that still lines up with everyone else's. And it never says a callsign and never calls CQ, deliberately: if it ever hits the air by accident, anyone listening hears it announce itself as a test recording, twice.
- **Heads-up: on the VOX field, V now toggles VOX.** Everywhere else in Home, V still hops you to the next slice. But standing on the field named VOX and pressing V used to move you off the very thing you were adjusting — I did it myself, because V obviously means VOX when VOX is what you're on. Now the letter agrees with the field, the way M already mutes on Mute, R toggles RIT on RIT, X on XIT, and Q on Squelch. If V-to-next-slice is in your fingers, it still works from every other field, and the Slice field is the dedicated place for hopping between slices.
- **T sets the transmit slice from the Transmit slice field now.** The Slice field taught you that T means "make this the transmit slice" — and then the field actually named Transmit slice didn't answer to it. It does now: T there does the same as Space, putting transmit on the slice you're hearing.
- **The Fix tool tests your receive audio now, as its very first step.** Don asked for this one, and he was right: the report you send to your radio's manufacturer never said a word about audio actually *arriving*. It listed nine settings — mutes, levels, a routing switch — every one of them something this application had asked your radio to be. So the report could be completely, verifiably correct while no sound had ever reached your computer. It said the plumbing was configured; it never said water came out. Stage 0 is called Audio setup and receive now, and it walks the whole receive path before anything else happens: the slice you're listening to, the radio's outputs and their levels, how the audio reaches you, and how many kilobits of it have really been crossing the network over the last half minute. Whatever you came to the Fix tool about, the receive evidence is in the document.
- **And it's the same test as the one in the Audio Workshop — not a copy of it.** This is the part I care about most. Audio Workshop, Diagnostics still has a button that answers "why is my radio silent" on its own, for when that's the only thing you want to know. Both doors now run the identical test, off the identical rule file, phrased by the identical code. When someone adds a new reason a radio can go quiet, it turns up in both places without anyone having to remember the second one. Two homes for one idea is how they quietly start disagreeing, and that's the bug I've been chasing all over this app.
- **A silent radio's most-forgotten cause is a real check now.** If everything else was fine, the old receive answer ended with a hand-written afterthought: "check the slice volume and that a slice is not muted." Good advice, sitting in a sentence with nothing behind it — no reading, no evidence line, nothing that would ever appear in a report. The slice is stage 1 of the receive walk now, with its own mute and its own volume read straight off the radio. A muted slice is silent *everywhere at once* — headphones, line out, front speaker and the audio sent to your computer — so it goes first, ahead of every output.
- **"Nothing is wrong" no longer means "we couldn't look."** The receive check lives in a text file you can edit. If that file goes missing, or your own copy of it can't be read, or it's there but empty — nothing gets checked at all. The answer you used to get in that case was that nothing was wrong with your receive audio. Which is the exact failure a diagnostic exists to prevent, happening inside the diagnostic. There are three answers, not two: nothing is wrong, something is wrong, and we could not check. You get the right one now.
- **The checks are called tests everywhere, because I only ever call them tests.** The tool said "checks" on its page and "tests" in the box that asks whether you want to stop — in the same sitting, about the same thing. Each string was written correctly on its own day, which is exactly how this happens. It's "test" now: Run this test, What this test does, Transmit tests, the report, the lot.
- **The menu says what you can do instead of what's behind the door.** Tools, Fix used to end with "Saved check runs", which tells you what's in there and not why you'd open it — and the reason you would is the one thing you can't guess: **you can pick a stopped run back up.** It reads **View or resume saved test runs** now, and every sentence in the app that points at it points at the new name.
- **The Fix tool now walks your whole transmit path, at the moment it has your radio keyed.** This is the one I'm happiest about, and it's embarrassing how long the two halves sat side by side without being introduced. Audio Workshop, Diagnostics has a thirteen-step transmit test in it — microphone, Windows level, computer audio, the encoder, the packets, what the radio says it heard, power out the back — and it's the sharpest thing in the app. It also can't transmit. Nothing in that room can key a radio. So three of its thirteen steps, including the two that matter most when nobody can hear you, said "transmit and run the test again" on every run there has ever been. Meanwhile the Fix tool keys your radio, sends it a tone at a known loudness, holds it up for eight seconds and takes your microphone out of the path so nothing depends on you speaking — and didn't run the test. Each one had exactly what the other needed. Now the transmit tests run that walk while the carrier is up, twice: once with the injected tone and once with your own voice, which differ in the one thing those two tests differ in. Instead of "three probes, no result," the report names the step where it stops and says the ones before it were clean. That's a diagnosis.
- **It's the same test as the one in the Workshop, the same way the receive one is.** One rule file, one set of words, two doors. Add a reason transmit audio can die and it turns up in both places, with nobody having to remember the second one.
- **You can change frequency without killing a test run.** The transmit tests key your radio for real, and until now the only way to move off a busy frequency was to abandon the run and start over. I never noticed, because I built and tested the whole thing into a dummy load, where the frequency doesn't matter and nothing radiates — the very thing that made the bench safe is what hid this. On a real antenna, finding a clear spot is the first thing you do before you key. Every test that transmits now has a "Change the frequency" button right beside "Change the transmit power," it opens the same frequency entry you already know, and you come straight back to the test you were on with the run untouched. The Run button then tells you the frequency it's about to use, so you hear where you are before anything goes out.
- **You can change the mode too — and what you hear back is the radio's word, not ours.** Same argument, one step along: a transmit audio test run in the wrong sideband measures the wrong thing, and on a real antenna somebody else hears it. The tests that put audio on the air now have a "Change the mode" button beside the frequency one, offering the four modes this kind of test actually runs in — LSB, USB, DIGU and DIGL. Not all twelve, on purpose: these are the ones with a real transmit audio path, which is the thing being tested. If your radio's parked in CW or FM, the picker tells you so and lets you move to one of the four or keep what you have — no pretend fifth entry. It won't touch anything while you're transmitting. And after you pick, the tool asks the radio what it's on *now* and tells you that — because this radio has been caught agreeing to a change and quietly not making it, and the one thing worse than a wrong mode is being told it's right.
- **The report says where you transmitted.** It didn't. A document describing an on-air transmission, meant to be sent to your radio's manufacturer, and it couldn't tell them the frequency. Every test that keys the radio now records the frequency, the mode and the antenna port it actually ran at, read at the moment of the transmission — so if you move to find a quiet spot halfway through, the report says which test ran where instead of quietly implying they all ran together.
- **The countdown before a transmission is a countdown again.** Three tones warn you before the Fix tool keys your radio, and those tones are the window where you can still stop it. The radio was coming up on the *second* tone — a third of the way in — and then playing the rest of the warning at you while you were already transmitting. Two numbers in two different parts of the app had to agree about how long the count was, and one of them had quietly changed months ago. They can't disagree any more: the part that keys the radio now asks the part that makes the sound how long it is, instead of remembering an answer.
- **Three keys that worked in secret are in the book now.** On the Slice field, every digit answers — a number past your last slice says "no such slice" instead of leaving you to wonder whether the key is broken. On the Classic frequency field, the minus key has one job and always has: reminding you that step entry uses plus only. All of them now show up when you press Shift slash, in the per-field help, and in the keyboard reference, instead of being keys you could only find by accident.
- **A slow connect can't freeze the whole app any more.** When a remote connect took its time — and one of mine took forty-five seconds, three times in a row — the entire application used to go silent and deaf with it: no speech, no keys, nothing to do but kill it. The waiting now happens behind the scenes, so your screen reader keeps answering, the Connecting window keeps talking, and Escape actually cancels. However a connect misbehaves, from now on you can hear it and you can leave it.
- **Reconnecting quickly got much less haunted.** Disconnect and reconnect in short order and you could meet a ghost of your own last session still holding your station name — at which point JJ Flexible Radio politely renamed you to get out of your own way, and the connect hung. It no longer mistakes stale leftovers for a real second operator, it now says a proper goodbye to the radio when you disconnect so less gets left behind, and when a radio does refuse to take your station name, the trace finally records the radio's answer instead of shrugging.
- **The connect announcement names the right account now.** Connecting to a radio on a friend's SmartLink account — Don's, in my case — the app correctly went through *his* account, and then told me it was using *mine*. The words and the wiring now come from the same place, so what you hear is what's actually happening.
- **Muting a slice tells you which slice.** "Slice B muted" — every time, from every door. Six different ways get you to a mute: the hotkey, the Audio menu, and the M key on four different Home fields. One of them named the slice and five of them just said "Muted," which on a four-slice radio answers the one question you weren't asking. The right wording had been sitting in the app for months, used by exactly one of the six.
- **The Mode menu tells you what mode you're in.** Open the Slice menu, go to Mode, and the mode you're on is checked — so your screen reader says "checked" as you arrive on it. It never was, which made that menu a trip you had to leave in order to learn anything: back out, press Ctrl+Shift+S, go in again. Every neighbouring menu had been marking its state all along.
- **The S meter on Home says what its number counts.** Space on the S Meter field used to read back whatever was on the display, which in dBm meant a bare "minus 97" — or "97", or "dash 97", depending on how your reader handles punctuation. Now it says "S meter minus 97 dBm", the same sentence `Ctrl+S` gives you, with the minus as a word. Over S9 it says "S 9 plus 20 dB" instead of a naked "plus 20". This got a lot more important when the unit became something you can set and keep.
- **The slice arrows go the way the list reads.** Down arrow walks you A, B, C. It used to be the other way round, which meant arrowing *up* to move *forward* through the alphabet. If your fingers learned the old direction, the Accessibility tab in Settings hands it straight back — one choice, and it covers both the Slice field and the Slice Operations field so the two can never disagree.
- **The Status window says which tuning mode you're in.** Classic and Modern give some keys different jobs, and losing track of which one you're in is easy. `Ctrl+Alt+S` now tells you, along with your coarse and fine steps in Modern, and which filter preset you're sitting on — or plainly says you're not on one. Both were on this window's original list and both got left behind when it was built.
- **Manage CW Messages actually opens now.** Tools then Manage CW Messages has been greyed out saying "not yet implemented" for longer than I'd like to admit, which meant that if you had CW messages on Ctrl+1 through Ctrl+7 you could send them and you could rebind their keys, but you could not change a single word of one, rename it, or add an eighth. It turns out the whole editor had been sitting there finished the entire time and nobody had connected the door to it. The list now reads each message with its key beside it, so you can hear which key sends which without opening anything.
- **Four keys in Logging mode stopped playing dead.** F6 switches between the radio pane and the log entry pane. Ctrl+Shift+N opens Log Characteristics. Ctrl+Alt+L opens the full log form — that one's on the Log menu now too. And Station Lookup's Log Contact button drops you into Logging mode with the call, name, QTH, state and grid already filled in. All four were documented, all four were bound, and all four did precisely nothing when you pressed them — which is the worst possible answer, because a key that does nothing is indistinguishable from a key you missed.
- **Firmware updates refuse over SmartLink out loud.** Firmware can't travel over SmartLink — it uses a connection SmartLink doesn't carry — and the only thing enforcing that was a greyed-out button that decided once, when the window opened, and never looked again. Reconnect over SmartLink with that window already up and the button was still live. Now the refusal is real, it happens where the update would actually start, and it tells you why instead of just being unavailable.
- **One setting stopped over-promising.** "Allow firmware updates without someone at the radio" read like a lock you were unlocking. There is no at-the-radio check for firmware yet, so it wasn't holding anything back — it's your answer for when that check arrives, and it now says so rather than implying a protection that isn't there.
- **Leftover speech waits its turn now.** When something interrupts your screen reader mid-sentence — you press Tune, you Tab to the next field — whatever was still queued up gets spoken again afterwards so you don't lose it. Good. The trouble was *when*: it came back the instant the interruption started, so you'd press Tune, hear "Tune on", and then sit through a reminder about saving your profile before anything about the tune itself got a word in. The leftovers now hang back for a beat, the thing you just did gets to finish its sentence first, and only then does the backlog follow. A quick run of Tab presses no longer re-reads the same leftover on every press either; it waits until your fingers stop and says it once. Nothing is thrown away, and nothing you do is slowed down — the answer to your keypress arrives exactly when it always did.

### The keyboard comes back on its own now, and you can ask what's on your screen {#keyboard-comes-back}

This one got me twice — once in April, once this week — and both times I was sure the app had frozen. I'd opened Select Radio, gone to connect to a friend's radio that wasn't answering, left the dialog sitting there a while, and then found my keyboard did nothing and my screen reader had nothing to say. So I killed the app. Both times.

It wasn't frozen. When I finally had someone look at the screen while it was happening, the dialog was right there, healthy, visible, waiting for me. The keyboard had simply gone somewhere else. Windows lets any program take the foreground once you've been idle for a few minutes, and something did — and nothing in the app ever looked to see whether the window you were in still had your keyboard. For someone who can see the screen that's a nuisance: the dialog's still there, click it. For those of us who can't, a window that stops answering is indistinguishable from a crash, and I treated it as one.

So now the app looks. If one of its dialogs is up waiting on you, and another program takes the keyboard while you weren't doing anything — no Alt+Tab, no click — it notices within a few seconds, takes the keyboard back, and tells you what happened: "Another program had taken the keyboard. You are back in Select Radio" — and it names the program when it can. A few things I want to be clear about, because taking the keyboard back is a serious thing for a program to do. It only does this when you were idle: if you went to your email on purpose, it leaves you there, every time. It only does it while one of its own dialogs is holding the rest of the app anyway, so there's nothing else of ours you could have been using. It never takes the keyboard from a password, security or permission prompt. And if something keeps grabbing it back, it gives up after two tries rather than fighting all evening.

**And there's a switch for it now.** Settings, Accessibility, "Take the keyboard back if another program takes it while you are idle." It's on, and I'd like it to stay on — I sat at the radio and watched it not fire while I was genuinely away from the machine for the better part of two minutes, which is exactly the case I was most worried about. But I told Don that if a window of mine ever came to the front when he hadn't asked for it, he should tell me and I'd switch it off, and I meant it. Now that doesn't need a new build to keep. If you do switch it off, it keeps watching quietly and notes in the diagnostic log every time it would have stepped in, so a session where your keyboard went missing can still be explained afterwards. Switching it off doesn't touch the other half of this, the one for a keyboard that's gone nowhere at all — if no window on your screen is taking input, a dialog of mine will still come back for you, because that case can't possibly be you having wandered off somewhere on purpose.

The other half is the question I most wanted to ask during both of those episodes and had no way to: what is actually on my screen right now? `Ctrl+J` then `Alt+W` answers it. You get a list of every window, as it was the moment you pressed the key, with the one that has the keyboard first and this app's own windows next. Each row says which program owns the window, in plain words where I know them, whether that program is still running and answering, and whether the window is a dialog blocking the rest of its program. If a window's program has exited and left it sitting there, the row says exactly that. And if the app has taken the keyboard back from something earlier in the session, the last row tells you who took it, from where, and when. It's a list, so you can arrow through it at your own pace, F5 looks again, Escape closes it. It changes nothing. It just tells you what's there — which, if you can't see the screen, is the thing you never get for free.
...
### The S-meter is honest now — and your readings will change {#smeter-tells-the-truth}

Here's an uncomfortable one, and I'd rather you hear it from me than discover it on the air.

The S-meter has been reading low. Not by much on HF — about half an S-unit — but by a *lot* above 30 MHz, where it's been three to four whole S-units short. Both problems are fixed, which means that starting with this build, the same signal reads stronger than it did last week. Nothing about your radio, your antenna, or the band has changed. The scale did.

Two separate things were wrong, and they'd been wrong for different reasons.

The first was a fudge. There's a worldwide standard for what an S-unit means — S9 is a specific amount of signal, and every S-unit below it is 6 dB weaker. Somebody, a long time ago, started from that standard and then moved the S9 mark 3 dB, so it took a slightly stronger signal here than anywhere else to earn the same number. No note, no reason, just a quiet nudge sitting in the arithmetic since the first day this code existed. It's Jim's code, from before I inherited any of this, and I have no idea what he was chasing. It's gone now, and S9 sits exactly where the standard says it should.

The second is bigger, and it only matters above 30 MHz. The standard actually defines *two* S9 marks: one for HF, and one for VHF and up that sits twenty decibels weaker — because a signal that would be a booming S9 on 20 meters is a completely different animal on 2 meters, and the scale is supposed to reflect that. This app only ever knew about the HF one and used it everywhere. So on 6 meters, and on 2 meters and 70 centimeters for those of you with an 8000-series radio, the meter has been reporting three to four S-units *under* what the standard calls that signal. It now knows which side of 30 MHz you're on and uses the right mark.

**What this means for you in practice.** Your S-meter reads higher. A signal you'd have called S7 on 20 meters is now S8. A 6 meter signal you'd have called S5 might now be S9. Your radio hasn't suddenly grown ears; it was being hard on itself, and now it isn't. If you keep a log with signal reports in it, reports from before this build and after it aren't on quite the same scale — the older ones run conservative.

**And the part that will look like a bug: JJ Flexible Radio and SmartSDR may now disagree on the same signal.** I don't know exactly what SmartSDR's scale does — I haven't put the two side by side on a bench with a signal generator, and I'm not going to guess at somebody else's arithmetic in a changelog. What I can tell you is that if the two of them used to agree, that agreement was resting on a 3 dB fudge, and they won't agree any more: expect roughly half an S-unit of difference on HF and a much bigger gap above 30 MHz. Neither one is broken. We now report the international standard, plainly and on purpose, and I'd rather be right and different than comfortable and wrong. Check my arithmetic if you like — the numbers are simple: S9 is minus 73 dBm below 30 MHz, minus 93 dBm above it, and every S-unit is 6 dB.

If you'd rather have a number with no scale politics attached at all, dBm is right there — see the next entry.

Old signal captures under Tools, Signal captures are fine and don't need re-reading. Every capture report has always written down the exact calibration it used, in the "How these numbers were taken" section — which was put there for a hypothetical suspicious reader and turned out to be for us. An old report explains its own numbers.

[Return to version headlines](#unreleased-headlines)

### The S-meter's units are yours to pick, and they stay picked {#smeter-units}

Press `Ctrl+J` then `Ctrl+S` and the S-meter switches between S-units and dBm. It says which one you landed on, and there it stays — for that radio, through restarts, until you switch it back. `Ctrl+S` then reads the meter in whichever unit you chose, and it names the unit every time, so there's never a bare number to squint at. "S meter minus 97 dBm," with the minus as a whole word rather than a hyphen, so there's no chance of it arriving as "97."

Why you'd ever want dBm: an S-unit is a blunt instrument. One S-unit is 6 dB, which is a factor of four in power hiding inside a single number. If you're swapping antennas, comparing two stations, or trying to tell whether that new preamp did anything at all, "S7" both times tells you nothing while the signal quietly doubles. dBm tells you. It matters a bit more after the fix above, too: the scale is trustworthy now, but it's still coarse, and coarse and honest is still coarse.

Here's the thing I got wrong and want to own. This switch has existed for years, buried where nothing but the Command Finder and a menu could reach it, and I'd forgotten it was there. For one test build I answered that by adding a *second* key — one that took a single dBm reading without changing anything — on the reasoning that a mode is a state you can be in the wrong one of. That reasoning isn't wrong, and it isn't the answer either. Don and I talked it through, and the answer is a mode you can hear, that stays put, and that Settings will show you: then you're never in it by accident. So there's one key and one unit at a time, and the second key is gone.

You'll also find it on the Radios tab in Settings, under Signal strength, remembered per radio by serial number — so the big rig you use for antenna work can sit in dBm while a second radio stays in S-units, and neither one disturbs the other. That's the half a key on its own can't give you: somewhere to *look* and see where things stand, rather than pressing something to find out. It's on the Operations menu as well, where it always was.

One warning worth having in advance: in the JJ key layer, plain `S` toggles the radio's own spectral noise reduction and `Shift+S` toggles the one that runs on your computer. So the units key sits one finger-slip away from two noise-reduction switches. All three are toggles and all three announce themselves by name, so if you land on the wrong one you'll hear it say so and a second press puts it back.

[Return to version headlines](#unreleased-headlines)

### Switching screen readers mid-session works now {#reader-swap}

This one cost me an entire evening, and the funny part is that the app had been telling me the answer the whole time — in a file I wasn't reading.

Here's what happened. I was working with NVDA, switched over to JAWS, and JJ Flexible Radio went strange on me. Arrowing through the frequency fields said nothing. Things I knew had been announced never arrived. And JAWS started reciting the whole nest of containers around whatever I landed on, every single press, which is what a screen reader does when an app has gone quiet and it decides to read the screen for itself. I spent hours convinced this was a JAWS problem.

It wasn't. JJ Flexible Radio worked out which screen reader you were running exactly once, when it started, and then never looked again for the rest of the session. I had launched under NVDA, so it was still cheerfully handing every announcement to NVDA — which had left the building. JAWS was never getting a word from us. Close the app, start JAWS first, open it again, and everything was perfect, which is the tell.

So now it keeps checking. Swap readers while the app is running and it notices within a few seconds, moves your speech across, and the new reader introduces itself — "Speech now through JAWS" — in its own voice, which is most of the message. Anything that was queued up for the reader that left gets dropped rather than replayed at you, because hearing a re-run of the last two minutes is not a welcome.

It works in both directions, it copes with a reader that crashes and restarts, and it copes with starting the app with no screen reader at all and turning one on afterwards. And if it ever can't tell what's running, it leaves your speech exactly where it is rather than guessing — a wrong guess here means silence, and silence is the one thing you can't troubleshoot.

[Return to version headlines](#unreleased-headlines)

### Push to talk stays keyed while you hold it {#ptt-stays-keyed}

Under some screen readers, holding Ctrl+Space was transmitting in stutters. Not "the announcement was wrong" — the radio itself was keying and unkeying three times before it settled into the transmission I meant. On the air. Mid-word.

The reason is a genuinely obscure difference between screen readers: some of them don't pass a held key through to the application at all. They send a rapid series of separate presses and releases instead, and from where JJ Flexible Radio sits that looks exactly like somebody tapping the key over and over. So it unkeyed, because you appeared to have let go.

There was already a fix in for this, and it was half right. It waited a moment after each apparent release to see whether you were really still holding — but it worked out how long to wait from the *wrong gap*. A held key has two rhythms: a long pause before it starts repeating, then a quicker steady beat. The old code learned the quick beat and then used that number for the long pause too, which is always about twice as long. So the wait ran out at the start of every press, every time, and the more you used push to talk the more certain it got.

It now treats those two gaps as the two different things they are, and it takes the long one straight from your own Windows keyboard settings — Control Panel, Keyboard, repeat delay — instead of a number somebody measured on one machine on one evening. If your repeat delay is set slow, the app waits longer, because on your machine it has to. It also double-checks with Windows whether the key is genuinely still down before it ever drops your transmission, and that check can only ever keep you on the air, never cut you off.

If you use a screen reader that passes held keys through properly, none of this changes anything for you: it stays completely out of the way until it has actually seen the stutter with its own eyes.
...
There's also a quieter fix in here. If you'd set up a default global profile that didn't exist on the radio yet, JJ Flexible Radio would create it for you as it shut down — handy, and it still does — except it would happily do that while another operator was connected, baking their slices into your profile forever. It doesn't do that any more.
...
Press `Ctrl+J` then `Alt+V` and JJ Flexible Radio says it: "Version 4.1.16.1024, Debug, built August 27, 2026." Version, whether it's a test build or a release build, and the day it was made. Short enough to read straight back to me in an email, and it works from anywhere, including from inside a dialog.
...
### Volume mode and pan mode are one audio layer, and the filter gets its own {#audio-and-filter-layers}

I could never remember why we had a volume mode *and* a pan mode. Turns out there was no reason — volume mode came first, pan mode came later and was built the proper way, and nobody went back. So they're one thing now: the audio layer. `Ctrl+J`, then `V`, then a letter for what you want to adjust — `Ctrl+H` headphone, `O` PC output, `M` mic, `L` line out, `C` compander, `S` processor, `P` pan — and Up and Down move it. Pan still answers Left and Right too, because for pan those words mean something. `Ctrl+J`, `Alt+P` still drops you straight onto pan.

Two things are different, and I want you to hear them from me rather than from the radio. First, headphone moved from `H` to `Ctrl+H`, because plain `H` now lists the layer's keys — in this layer and in every layer we build from here on, so you never have to remember which layer uses which help key. Second, Escape now puts back *everything* you moved since you entered, and says so — "All changes reverted" — rather than reciting the values, since where you are going back to is where you just came from. Volume mode used to keep your changes on Escape, which meant "oops" had no key. It does now. Enter keeps the lot, as before. And `Shift` with a slice letter jumps to that slice without leaving the layer — pan follows you across.

Then I sat down at the radio and pressed it, and it was thin. It couldn't change the volume of the slice — the one volume most of us mean, and a different thing entirely from how loud your computer plays. It couldn't mute. And it could set how loud PC audio would be while having no way to turn PC audio on. All of that is in now: `V` is slice volume, `Ctrl+M` mutes the slice you're on, `Ctrl+P` turns PC audio on or off, and `Ctrl+B` switches binaural receive — the radio's stereo widening, which used to have a control until the move lost it. The switches flip with one press and say which way they went, and Escape puts them back along with everything else. Entering the layer got a lot quieter too: it says where you are and what's picked, and stops. `H` opens the layer's keys as a list you can arrow through — the same list the JJ key's own `H` gives you — instead of reciting every one of them in a single breath, which is what it did the first time I pressed it. And `Shift slash` opens the JJ key explorer with this layer's branch already open. The letters for the four new ones are a first draft; I may move them once I've lived with them for a bit.
...
### Pan says the number, and only the number {#pan-says-the-number}
...
So now it's "Pan 40", at every level, every time.

My first pass kept the word and put the number in front of it — "Pan 40, slightly left" — on the theory that the word is the friendly half while you're still learning where things sit. Then I listened to it while actually working the control, and a slider is not a thing you hear once. You nudge, and nudge, and nudge again. Every one of those nudges was paying for a word that told me less than the number sitting right next to it.

There's a second reason, and it's the one that decided it. Pan runs 0 to 100, exactly like slice volume, and slice volume has never announced a unit — it just says the number, and you know the scale because you're the one turning it. Pan saying anything extra would have made it the odd one out among your own controls.

The number is also the one you can scribble on the back of a QSL card and dial in again tomorrow night, which is the entire reason to have one. 0 is hard left, 50 is center, 100 is hard right. Same as it ever was, said in the one form that can tell 15 from 34.
...
### The JJ layer waits for you after an unknown key {#jj-layer-waits}

This one is small, and it bothered me more the longer I looked at it.

Press `Ctrl+J`, then a key the layer has no command for, and it used to say "Unknown command. Press H for help." Perfectly reasonable sentence. Completely useless advice — because by the time it finished saying it, the layer had already closed behind you. Press `H` as instructed and you got whatever `H` does wherever you were standing. The one thing you were told to do was the one thing that couldn't work, and you were already lost when you were told it.

So the layer waits now. After an unknown key it stays open for exactly three keys: `H` and `Shift slash`, which both read you the command list, and `Escape`, which closes it. Press anything else and the layer is already gone — that key does its ordinary job, nothing swallowed.

Three and no more, deliberately. A layer that quietly keeps hold of your keyboard is a trap, and there's no way to see whether you're in one. These three all lead *out* — two to the list, one to the door — and the layer only ever lingers immediately after telling you it's doing so. You can't end up held somewhere you didn't ask to be.
...
### The layers listen to your verbosity setting now {#layers-honour-verbosity}

Confession time. The new audio and filter layers were built in a hurry, and in the hurry nobody told them about your verbosity setting. So every time you opened the audio layer — on Chatty, on Terse, it didn't matter — you got the full roll-call: "Audio layer. Ctrl+H headphone, P PC output, M mic, L line out, C compander, S processor, Ctrl+P pan. Up and down adjust. Enter keeps it, Escape puts it back, H lists the keys." Helpful the first time. Punishing the fortieth. I heard myself say "damn, we speak a lot in these layers," and I was on Terse when I said it.

Here's the thing I decided along the way, and I want to say it out loud because the other option is tempting. I could have made the app *learn* — say less once you've heard the lesson a few times. I'm not doing that, ever. An app that behaves differently from last time, for a reason you can't see, is worse than a chatty one, and when sound is your only feedback, an unexplained change in what the app says is indistinguishable from something being broken. You already own a control for this, you can predict it, and it's one key: `Ctrl+Shift+V`.
...
One promise underneath all of it: turning verbosity down never leaves you with nothing. If you've switched your earcons off — all of them, or just the command tones — there's no thunk to hear, so the app speaks the short form instead, at every level. A key you pressed always answers. And the level only ever changes what the app *says*, never what it *does*. After an unknown JJ key the layer still waits for `H`, `Shift slash` and `Escape` exactly as it does on Chatty; it's just that on Terse the thunk is the thing telling you so.
...
### Audio Workshop has its own letter on the Audio menu {#audio-workshop-w}

The Audio menu has two rows that start with "Audio" — Audio Devices and Audio Workshop — and a single letter can't tell them apart. Last time round I made the menu's shape stop changing depending on whether a radio was connected, which at least made `A` behave the same way every time. It still took two presses to reach the workshop.

Audio Workshop now has a mnemonic of its own. With the Audio menu open, `A` goes to Audio Devices and `W` goes to Audio Workshop. `W` for Workshop, which you'll remember without trying.

Nothing is renamed — I'd floated calling Audio Devices something else, and this is better: no help page to rewrite, no muscle memory broken, and nothing for you to relearn. And `Ctrl+Shift+W` still opens the workshop from anywhere without going near the menu at all.

[Return to version headlines](#unreleased-headlines)

### Binaural has a control again {#binaural-has-a-control}

Binaural receive is one of those things your Flex has always been able to do and hardly anybody talks about. Ordinarily both your ears get the identical signal, which is why a busy band can feel like it's happening inside your skull. Switch binaural on and the radio spreads it out — same signal, more room, and on a crowded band it's genuinely easier to pick one voice out of the mush. It costs nothing and it's worth trying at least once.

Here's the part that isn't flattering. It has been wired and working in JJ Flexible Radio this whole time. Jim had a control for it, the control didn't come across when we rebuilt the screens, and the radio's end of it never stopped working — so the app could set binaural, and nothing on your screen could ask it to. Last time round I gave it a key, `Ctrl+J` then `V` then `Ctrl+B`, and called that fixed. It wasn't, really. A key you have to already know about is not how anybody discovers a feature; you find things by arrowing past them.

So it's somewhere you'll actually trip over it now. **Binaural** is a checkbox in Home's Audio and Slice group — `Ctrl+Shift+U` and arrow down past Volume and Pan — and there's a **Binaural** row on the Audio menu. F1 on the checkbox tells you what it does, in case the word alone doesn't. The key still works exactly as it did.

Two things worth knowing. It's a property of the radio rather than of one slice, so it changes everything you're listening to at once — unlike Pan, sitting right above it, which moves only the slice you're on. And whichever of the three you use, it says the same two words: "Binaural on," "Binaural off." One switch shouldn't describe itself three different ways depending on how you reached it, and there's now a test that won't let it.

I went looking for a Settings entry too, and decided against it. Settings is where you keep decisions; binaural is a thing you flip while you're listening, hear the difference, and flip back. It belongs where the sound is, not in a dialog you have to close before you can hear the result.

[Return to version headlines](#unreleased-headlines)

### The tracking notch filter is back {#tracking-notch-returns}

**Press `Ctrl+J` then `Alt+N`, or find Tracking Notch Filters on the DSP section of Home.** It's also the second half of a story I told in the last release about the equalizers, and I'm not thrilled about the pattern.

Here's what a tracking notch is for, in case it's new to you. Auto Notch goes hunting for a carrier on its own and kills it, and when it works it's wonderful. When it doesn't — when it keeps latching onto the wrong tone, or lets go every time the signal fades under it — a tracking notch is where you point at the offender yourself. Tune onto the birdie, press Add, and the radio drops a notch right there. You pick how wide it is and how hard it bites. And there's a setting called Keep: turn it on and the radio holds that notch through band changes and through turning the radio off and on, which is exactly what you want for the switching-supply whistle that shows up in the same spot on 40 every single evening.

Your radio has been ready to do this the whole time. The app has been telling your radio "tracking notches on" at every connect for as long as I can remember. There has simply been no way for you to place one, because the window that used to do it was a Windows Forms window, it got replaced by a modern one during the rewrite, the old one was deleted, and nobody ever hooked up the new one. Two perfectly reasonable steps with a hole between them, and the feature fell straight through it. Nothing crashed, nothing logged an error, no test went red — the code was all still there and all still correct, just not reachable from anywhere. Sound familiar? It's the same shape as the equalizers, and it turned out both of them had been sitting on the same never-displayed panel.

So I went looking on purpose this time instead of waiting to trip over the next one, and I found a couple more that I'm still thinking about — a pan-adapter list and a watt meter setup window, both in the same state. I'd rather tell you that than quietly fix one a release and let you wonder what else is missing.

While it was open on the bench I fixed some things about the window itself that would have bothered you. Typing a width and pressing Enter used to snap the box back to the old number, as though the radio had refused it — it hadn't, the window was just reading the value back before the radio had a chance to answer. The arrow keys change width and depth and now say the new number out loud, instead of silently changing text you can't see. Remove leaves you on the next notch rather than dumping you out of the list, so clearing three in a row is three presses. And if you open it with no notches set, it tells you that it's empty and what to press, rather than handing you a silent list box and letting you wonder whether it had failed to load.

[Return to version headlines](#unreleased-headlines)

### VOX has its settings back {#vox-settings-return}

Don raised this one twice, and the second time he put it exactly right: "Turning on VOX I don't believe works at this point, and that may be merely because it can't be configured." It couldn't, and that's why it didn't.

VOX comes down to two numbers. **Gain** is the threshold — how loud your voice has to be before the radio decides that's speech and keys up. **Delay** is the hang time — how long it stays keyed after you stop, so it doesn't drop out between your words. Your radio has always had both, and the app has always known how to set them; the window that let you reach them was one of the old-style windows that got deleted during the rewrite, and the new Home never picked them up. Same story as the equalizers and the tracking notch — I'm as tired of telling it as you are of hearing it — but this one has a twist, so read on.

**Where they are now.** Open the Transmission group of Home (`Ctrl+Shift+X`) and they sit right after the VOX checkbox: **VOX Gain**, 0 to 100, and **VOX Delay**, in milliseconds, up to two seconds. Arrow them, or type a number and press Enter. They're there whether VOX is switched on or not, and that's deliberate — you set the threshold first and arm VOX second, and making you switch on an unconfigured VOX to get at its settings is the very problem Don was describing. In CW they step out of the way entirely, because the VOX checkbox means break-in there and the two numbers don't apply. They're in the audio layer too: `Ctrl+J`, `A`, then `X` for gain and `D` for delay, right next to mic level, the compander, and the processor, so you can trim VOX without leaving whatever you were doing. Each one tells you if VOX is off after it says the number, so you'll never sit adjusting a threshold that isn't in play.

**The twist.** While wiring the delay back up I checked how the app converts your milliseconds into what the radio actually takes, and it has been wrong since the first version. The radio counts hang time in steps of 20 milliseconds; the app was doing its sums with 50. So when the app told you "two seconds," the radio had been given 800 milliseconds. Worse, the top sixty percent of the radio's range was simply out of reach — nothing you typed could get you past that 800. And if you'd set the delay to its maximum in other software, the app would have read it back as five seconds, a number no VOX has ever needed. That five was the giveaway: it isn't a setting anyone would choose, it's what the wrong arithmetic prints when a radio is sitting at its real maximum of two seconds. Jim had the range exactly right — zero to two seconds is precisely the radio's range — and one constant wrong, which is the most forgivable kind of bug and the hardest to see, because nothing ever failed. The number you heard just wasn't the number on the air. It is now, and there's a test that reads the radio library's own documentation and refuses to let the two drift apart again. The plain-text profile export had the same sum in it, so if you'd saved one, the VOX delay line in it was wrong too; new exports are right.

One more small thing for the delay: Shift with an arrow moves most numbers by one, but the radio can't do one millisecond — it moves in twenties. So on this field Shift moves 20, which is the finest step there is, and every number you hear is one the radio can actually hold.

Don is receive-only for a while, so he hasn't been able to tell me VOX keys up for him yet. If you use VOX, I'd like to hear how the defaults suit your voice.
...
The radio made RF. Not much — around a tenth of a watt, three times in a row, real signal genuinely leaving the radio. And JJ Flexible Radio said **0 watts**, which is exactly, character for character, what it says when you aren't transmitting at all.
...
### Warnings can duck under the band — and you hold the fader {#warning-dip}

Here's a problem every one of us knows: the app has something urgent to tell you, it plays the warning sound, and 40 meters is busy eating it alive. A warning you didn't hear is a warning that didn't happen.

So there is a setting that dips radio audio coming through your computer while a warning sound plays — four decibels by default, which is "steps politely aside" rather than "slams the fader" — and glides right back up the moment the warning is done. Only warnings do this. Keyclicks, confirmations, dialog dings, the whole chorus of ordinary sounds: none of them will ever touch your receive audio. And the dip physically cannot get stuck on — it's built so the audio always comes home to exactly where it was, even if something goes sideways mid-warning.

Now, the part I care about most: **it's entirely yours to control.** Under **Settings**, then **Notifications**, there's a section called **Warnings and Radio Audio** with a switch and a depth. Turn it off and your band is never touched, full stop. Or keep it and set the depth anywhere from 0 to 12 decibels — maybe you copy weak CW and want the barest nudge, maybe you're in a noisy shack and want the warning to arrive like it owns the place. Both changes take effect the moment you make them: pick "Warning alarm" in the Per-Sound Loudness section right below, press Play with radio audio running, and hear exactly what a warning will do to your band. No guessing, no saving-and-waiting.

One thing changed after I had lived with it for a while: **it now starts switched off.** The dip turned out to draw more attention to itself than the warning it was making room for — a rare movement in your audio is more noticeable than a constant one, it turns out. So nobody gets it until they ask for it. If you had already turned it on and liked it, nothing here reaches back and undoes your setting.

And there is a second knob beside the depth now, because when I said the dip bothered me, what I actually went looking for was a hang time and there was not one — the timing was welded shut. **Quick** is how it shipped, snapping down and back with the sound. **Gentle** eases down and eases back. **Lingering** stays down a moment after the warning and returns slowly, so the band breathes instead of stepping. On all three a deeper dip comes back more slowly than a shallow one, so the return never lurches, and nothing takes longer than a second and a half to bring the band all the way home. My hunch is that a dip you notice more than the warning is one that moves too fast rather than one that goes too deep — and that is now something you and I can settle by ear.

If you operate remote, this section is especially yours. At home you can always switch PC audio off and lean on the radio's own speaker — but over a remote link, PC audio *is* the radio, so whether warnings get to dip it is a decision only this switch can make. That's exactly why the switch had to exist before this feature could ship.

### Every sound has its own volume knob now {#per-sound-loudness}

The app's sounds are balanced against each other on purpose — warnings loud, confirmations modest, keyclicks barely there. But that balance was tuned by my ears, and yours are different. Maybe the connect chime lands piercing to you. Maybe the problem-recorded tone vanishes into your noise floor. Until now your options were "live with it" or "email me," and only one of those scales.

So: **Settings**, then **Notifications**, then **Per-Sound Loudness**. Pick any sound the app makes from the list — they're grouped by family, and each one *plays as you land on it*, so you can hunt down "that one beep that's been bugging me" by ear instead of guessing from names. Then trim it: up to 3 decibels louder, or up to 12 quieter, arrows or typed straight in. It applies as you adjust and there's a Play button to hear it again, so the whole decision happens in your ears where it belongs. Only the sounds you touch are remembered; everything else keeps its shipped balance.

Two guardrails worth knowing about. A trim can't reach silence — twelve down is quiet, not gone — because losing a warning should never be one slider away, and the on/off switches above the list are the proper tool for sounds you don't want at all. And it won't go louder than 3 up, because past that a sound doesn't need a trim, it needs to be redesigned, and I'd genuinely rather hear about it.

[Return to version headlines](#unreleased-headlines)

### Ctrl silences the Morse now, too {#ctrl-silences-cw}

You already press Ctrl to shut your screen reader up. You don't decide to — your thumb just does it. Until now that reflex only half worked here: speech stopped and the CW notifications kept right on keying, which is arguably worse than a key that does nothing, because something you trusted did *part* of what you meant. One tap of either Ctrl now stops the Morse along with the speech, wherever you are, whatever has focus. Your screen reader still gets the key exactly as before — this rides alongside it, it doesn't get in its way.

**And CW notifications stopped queueing up on you.** Morse is slow — that's rather the point of it — so a pile of queued notifications used to mean sitting through announcements about slices you'd already left. Now a newer message simply replaces the one still waiting. Arrow across four slices and you hear the one you landed on, not the tour. The message being keyed finishes its current character before handing over, so you never hear a chopped half-letter that reads as the wrong letter — and the connect and sign-off prosigns always play out whole, because BT and SK are the channel's manners and manners don't get interrupted. One more consequence you might notice: the app no longer re-keys a message when nothing changed. If the census says "SL 4/4" and the slices haven't moved, it's said its piece.

[Return to version headlines](#unreleased-headlines)

### You get to decide how quick quick really is {#quick}

There's a new setting in **Settings > Accessibility** called Double-Tap Tolerance. It has four choices — Quick (250 ms), Normal (500 ms), Relaxed (750 ms), and Leisurely (1000 ms) — and it controls how sensitive JJ Flexible Radio is when detecting a double tap. If Quick is selected, you have 250 milliseconds between presses to register a double tap. Select Leisurely and you have a full second. A longer tolerance may help folks with dexterity or fine motor control challenges to successfully double-tap when that was impossible before. On the other hand, if you're quick on the draw, by all means set it fast. The power is in your hands now.
...
Remember, you don't need to be on the field that says "squelch" in Home to toggle it. Press Q from any Home field to toggle squelch without arrowing — it's the same universal pattern as the other single-letter toggles. JJ Flexible Radio stores your last squelch level, so when you toggle squelch back on, it returns to whatever you set previously.

[Return to version headlines](#unreleased-headlines)

### Network Tab Configurability {#network-tab-configurability}

JJ Flexible Radio now gives you full control over the three SmartLink networking tiers needed to conquer difficult network topologies. If you're behind network address translation — most of us are — SmartLink can bust through. That's awesome, but until now screen reader users had to wrestle with SmartSDR's inaccessible GUI to make the radio-side changes SmartLink needs. The Network tab in Settings now does the following:

- **Manual port forwarding** is the sovereign option — you set the port, you know what's happening. Use this if you want to configure your router to open specific ports yourself. The port-forward setting you change from JJ Flexible Radio tells your radio which external port to advertise for UDP and TCP connections. You also have to use your router's admin pages to forward the matching ports. You **can** pick any external port you like — that lets you put multiple radios on the same network and still use SmartLink. Set up two port-forward rules on your router: rule 1 forwards your chosen external port over TCP to port 4994 on the radio. Rule 2 forwards your chosen external port over UDP to port 4993 on the radio. Sound complicated? It can be. If you're not comfortable with this process, you now have two more options, and JJ Flexible Radio will try them both.
- **UPnP** is the convenience option. Turn UPnP on and JJ Flexible Radio asks your router to set up port mapping automatically. Some operators are uncomfortable with UPnP because it can be an issue for security-conscious network setups — your choice, your network.
- **Hole-punch / extended reach** is the last-resort option for hard-to-reach or hard-to-configure networks. If UPnP and manual port forwards both fail, JJ Flexible Radio will try to bust a hole (legally, I promise) through your NAT so that SmartLink can reach your radio.
...
So: the radio list now honors Enter. Arrow to a radio, press Enter, you're connecting — exactly what the announcement has promised all along. And when a remote search finishes, focus lands directly on the first radio in the list, so what your screen reader announces and what Enter acts on are the same thing. If a stray press does land on Remote right after a search finished, JJ Flexible Radio just says your remote radios are already listed and puts you back on the list instead of starting over.

[Return to version headlines](#unreleased-headlines)

### Remote, Before You Ask {#remote-first-startup}

If your radio lives somewhere else — a friend's shack, a remote site, the other end of a SmartLink — then every session starts the same way: open JJ Flexible Radio, wait, press Remote, wait again. That first press is pure ritual. You were always going to press it.
...
It knows better now. The selector opens with every radio this copy of JJ Flexible Radio has ever met, and each row tells you where it stands: "6300inshack, FLEX-6300, remote via SmartLink" if it's there right now, or "6300inshack, FLEX-6300, offline, last seen remote via SmartLink, last seen 3 days ago" if it isn't. Rows go live as discovery finds them, and a radio that switches off doesn't vanish from the list any more — it just goes quiet and says so.

Press Enter on an offline radio and you won't get a dead end. If it was last seen through SmartLink, JJ Flexible Radio starts looking for it right then and tells you so. If it was on your local network, it says the honest thing: it's not here, it may be powered off.
...
Here's something that was quietly true and quietly invisible. If your Flex sits on your own network and you've also registered it with SmartLink, it's reachable two different ways. JJ Flexible Radio knew this perfectly well and never mentioned it — local discovery shouted first, won the row, and the SmartLink identity was thrown in the bin. You'd see "local" and that was the end of the conversation.
...
There's also a line under the radio list showing the SmartLink account in play, and it's a real stop on the Tab key rather than decoration you can't reach. And when you press Remote, JJ Flexible Radio now says which account it's signing in as before it goes — "Connecting to SmartLink as yourname@example.com." If you've ever had two accounts and stared at a radio list wondering whose radios these were, that one's for you.
...
- **A firmware update that dies mid-send now says so.** If the radio closes the connection while the firmware file is still uploading, JJ Flexible Radio tells you right then — "the radio closed the connection, the update was not applied" — instead of cheerfully waiting for a restart that isn't coming. The watcher still reports what version the radio comes back on, so you always hear the end of the story.
- **SmartLink registration tries twice before complaining.** The SmartLink server sometimes refuses a perfectly good registration on the first attempt and accepts the identical one a moment later. JJ Flexible Radio now quietly retries once before telling you something went wrong, and tells you when it's doing so.
...
The Connecting screen now does what it should always have done. Press Escape and JJ Flexible Radio cancels the connection attempt right away. Click the X close button, same thing. Either way, your screen reader hears "Connection attempt cancelled" and you land back where you started.
...
One more thing in the same neighborhood: if you've ever heard the 73 prosign play twice when you closed JJ Flexible Radio while connected, that was a small bug where the disconnect handler and the app-shutdown handler each played it. Disconnect now claims the prosign for the session and shutdown steps aside, so you hear 73 once — the way Jim originally intended.
...
Until now the only way to find out inside JJ Flexible Radio was to key up and watch
...
- **Only the noise floor.** JJ Flexible Radio can hear your interface but nothing is
  ...
  on. JJ Flexible Radio says so in those words.
...
**JJ Flexible Radio now handles Windows microphone privacy the way Zoom does.**
...
If it's off, JJ Flexible Radio now says so by name, tells you which switch it is, and
...
- **Devices say how they're attached** when Windows tells us plainly — USB,
  Bluetooth, HDMI. When it doesn't, JJ Flexible Radio says nothing rather than
...
- **A saved device that isn't plugged in stays visible.** It sits at the top of
  ...
  selected and JJ Flexible Radio keeps it saved for when you plug it back in; pick
...
strange. It resets itself the next time you start JJ Flexible Radio, because a
...
The application you launch is now called `jjflexible.exe`. It used to be `JJFlexRadio.exe`. That's the whole change on disk, and for almost everybody it's invisible — the installer swaps your Start Menu and desktop shortcuts over to the new file and cleans the old one out, so you launch JJ Flexible Radio the same way you always have and land in the same place.
...
When you run the new Setup on a fresh Windows machine, JJ Flexible Radio just installs. No more "this app needs Microsoft .NET 10 — please install it first," no chasing a separate download from Microsoft's website, no extra UAC prompt for a runtime installer that may or may not read well in your screen reader. Hit Setup, accept the install location, you're in. JJ Flexible Radio carries its own copy of the runtime now.

The trade is download size. The new installer is about 55 MB instead of about 10 MB, because the runtime is bundled inside. You only download it once per release, so the cost is a single concentrated hit instead of a multi-step scavenger hunt the first time. If you already have .NET 10 installed for another app, no conflict — JJ Flexible Radio runs on its own bundled copy and minds its own business.

Why this mattered enough to do: the "you need a separate runtime" experience for someone using a screen reader on a brand new shack PC was a wall. Microsoft's runtime download page wasn't always clean to navigate; the runtime installer occasionally threw a UAC prompt that read poorly under JAWS; and after all that work the original Setup might still demand a different prerequisite. Installing JJ Flexible Radio should be one step. Now it is.
...
Here's a bug with a long reach. JJ Flexible Radio tracked your slices by their position in a list, while the radio names them by letter. Alone on your radio those two always agree. Add MultiFlex — another station holding slices on the same radio — or churn slices for a while, and they could drift apart. The symptoms were maddening precisely because they were rare: jump to slice D and land somewhere else, change mode from the menu and watch it land on a slice you weren't listening to.

The letter is the identity now. When you say D, JJ Flexible Radio finds the slice the radio calls D — not the fourth thing in a list that may or may not start where you think. Announcements always speak the radio's true letter, so what you hear, what you press, and what the radio does are finally the same conversation.
...
- **The router rule, read aloud, verbatim.** When the evidence points at your router, JJ Flexible Radio builds the exact rule from what the radio itself advertises — external ports, the radio's address, the fixed internal ports — and speaks it. Nothing for you to reconstruct, nothing typed from anyone's memory.
...
- **Saved test runs have a home: Tools, Fix, then View or resume saved test runs.** Every saved run is listed there, newest first, each line leading with its test ID. Open one and the full report reads exactly like the live page, with a copy button. Export it as a web page or plain text to attach to an email — and if you want a PDF or a Word document, any converter that eats web pages will happily oblige. Delete the ones you're done with; the newest two hundred keep themselves. A run that stopped part-way says so in the list, and opening it leads with what's changed on your radio since it stopped — "Tune power changed from 10 watts to 100 watts" — naming the exact stage to run again, so you never trust a measurement your own settings have quietly outgrown.
...
You know what I hate about 60 meters? The channels, before I got my Flex, I was terrified to get onto 60, mainly because I was terrified that I'd be transmitting out of band. Fear no more Mr. NER, fear no more. Jump to a 60-meter channel and JJ Flexible Radio automatically switches you to USB — because that's what the FCC requires on those channelized frequencies in the U.S. If you land on the digital segment, it'll set CW mode. No more accidentally transmitting in the wrong mode on 60 meters. You'll hear a quick announcement telling you what mode was set and which channel or segment you're on. We will use the same approach for custom band segments that are available in other countries. This will have me and good ol Claude delving through pages and pages of radio regulations from lands near and far, but we're up for it, because we know that JJ Flexible Radio will be a worldwide success once it lands in operator shacks worldwide.
...
We added a new toggle in Settings → Notifications: "Show panadapter." Hide the visual panadapter if you don't use it and want JJ Flexible Radio's window and tab chain to be more clean. Show it if a sighted helper is looking over your shoulder. Your choice is saved.
...
- **Wide-open filter presets**: JJ Flexible Radio's filter preset now gives you a "wide open" mode which can give you some pretty incredible sounding receive audio on your end. Wide open equates to 4 kHz on SSB, 2 kHz for CW, 12 kHz for AM, and 16 kHz for FM. Sometimes you just want to hear everything your receiver can hear and that's OK. Have everything then!
...
- **Audio Peak Filter mode guard**: APF is a CW-only feature, but JJ Flexible Radio used to happily toggle it in SSB mode without doing anything. Now if you try it outside CW, you get a clear message: "Audio Peak Filter is only available in CW mode."
...
- **QRZ to HamQTH auto-fallback**: If QRZ login fails three times in a row, JJ Flexible Radio silently switches to the built-in HamQTH account so lookups keep working. You get a one-time notification explaining the fallback. No more silent lookup failures when your QRZ subscription lapses.
- **HamQTH built-in account**: If you select HamQTH as your callbook but don't have personal credentials, JJ Flexible Radio falls back to its built-in HamQTH account automatically.
...
This is the one I've been building toward. JJ Flexible Radio now has a dedicated Logging Mode — press Ctrl+Shift+L from anywhere in the app and you're in a clean, focused QSO entry screen. No menus full of radio controls you don't need mid-QSO, no hunting for the right form. Just you, your log, and the radio.
...
If you've been opening JJ Flexible Radio just to log a few CW QSOs and wished the UI would get out of the way, this is that update.

[Return to top](#top) · [Jump to versions](#versions)

## 4.1.11.0 — Classic/Modern Mode, Auto-Connect & Audio Fix {#v4-1-11-0}

This one's about giving you choices. JJ Flexible Radio now has two UI modes — Classic for the "if it ain't broke" crowd, and Modern for those of us who want a cleaner, more accessible interface that's built for screen readers from the ground up. Plus, auto-connect means the app just connects to your radio when it starts, and we finally fixed the "why is this so quiet?" WAN audio problem.

### Classic vs. Modern Mode

This is a big one for how JJ Flexible Radio feels day to day. You now have two ways to use the app:

- **Classic mode**: Everything you're used to. Same menus, same layout, same muscle memory. If you've been using JJ Flexible Radio for years and you like it the way it is, Classic is your friend. Nothing changes, nothing moves, nothing breaks. Don loves Classic mode, and honestly, we respect that.
...
- **Seamless auto-connect**: Pick your radio, right-click, "Set as Auto-Connect," and you're done. Next time you launch JJ Flexible Radio, it connects automatically. Works for both local radios and SmartLink remotes.
...
- **Saved SmartLink accounts**: After logging in, JJ Flexible Radio asks if you want to save the account. Give it a friendly name like "Main ISS Flex Radio" (I can dream, right?), or "Club Station" and next time you click Remote, just pick it from the list. No more hunting for passwords or waiting for two-factor codes while your DX window closes.
...
- **Automatic refresh**: When your session expires (they do, eventually), JJ Flexible Radio quietly tries to refresh it. If that works, you won't even notice. If it fails, you'll need to log in again, but your saved accounts stick around.
...
- **No more "access denied"**: Moved the browser cache folder to your AppData so it stops complaining when JJ Flexible Radio runs from Program Files.
...
- **64-bit and 32-bit support**: JJ Flexible Radio now builds for both x64 (64-bit) and x86 (32-bit). The installer names include the architecture suffix so you know which one you're grabbing.
