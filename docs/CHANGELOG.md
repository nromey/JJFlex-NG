...
- **The transmit checks stop throwing you back to the top of the page.** Every button on that page rebuilds it, and that part is honest — what the page says genuinely changes when you press something. What wasn't honest was leaving you at the top afterwards with no word about it, so every single press cost you the press plus the walk back down to where you already were. For a whole run. Now a press lands you on what it produced: run a check and you land on that check's own heading, which says how it went; answer "can you hear the radio right now?" and you land on your own answer, read back; press a one-press fix and you land on what the setting actually became. That last one is worth saying twice — the fixes have always read the change back to make sure it really took, and that sentence was being written down and never spoken.
- **A check you just ran no longer sounds like one you didn't.** When a check passed, the page was helpfully moving you along to the next one. Which meant the first thing you heard after running the audio setup check was the *next* check announcing itself as "not yet run" — and the result you pressed the button for went past unheard. Sitting at the radio, that sounds exactly like being told you hadn't run it at all, and I spent a while convinced that's what it was saying. You stay on the check you ran now, you hear what it found, and you move on when you press "Next" — a button that was already sitting right there and is the better way round anyway.
