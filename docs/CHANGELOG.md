...
The workshop also grew a live mic reading you can actually sit on. It's a read-only text field right before the Start button: land the workshop, Shift+Tab once, and there's your level — "just right, peak minus 11 dBFS" — updating continuously while you transmit. Because it's a real field, your screen reader's own read-current-control command re-reads it as often as you like, with no special hotkey to remember and no speech firing when you didn't ask. And when the workshop opens, focus is already on Start Audio Check: if you're set up, Enter starts the test — zero navigation for the common case.
