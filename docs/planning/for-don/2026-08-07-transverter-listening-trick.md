...
- **Don ranks the in-radio path ABOVE external SDR** for audio fidelity judgment, on three grounds: variable SDR receiver quality, inaccessible SDR bandwidth controls, and band conditions (QSB/QRM) that can block the check outright. This is in tension with the plan's ratified honest-claim language ("an SDR connected to a radio on a real antenna remains the best test, full stop") — see plan §4 for how the tension resolves.
