...
- **PC AUDIO SHOULD BE REMEMBERED PER RADIO (Noel, 2026-08-12, found by turning it on five times in a row during ghost testing).** **Verified: PC audio state is persisted nowhere** — every connect starts with it off, so an operator whose radio is only reachable remotely must re-enable it every single session. Don's entire situation, every day.
  - **Home is `Radios/RadioConfig.cs`**, which is serial-keyed and already carries exactly this shape of setting — the Workshop track added `AudioCheckPowerMode` and `AudioCheckLowPowerWatts` there on 2026-08-11 (lines ~207, ~217). Per-radio is the right grain and it is free: [[project-per-radio-config-serial-keyed]]. A radio at the far end of a SmartLink hop is *always* a PC-audio radio; a rig on the desk with headphones plugged in may never be.
  - **Recommended shape — one setting, three values, default the adaptive one.** *Remember last* (default), *Always on for this radio*, *Always off for this radio*. Remember-last is the friction-tax answer and covers most operators with zero configuration. But **"always on" is not redundant with it**: remember-last faithfully carries forward an accident, so if PC audio was off last session because something hiccuped, every later session inherits that. An explicit always-on is a declaration of intent that survives a bad session — which matters precisely for the remote-only operator who cannot hear anything without it.
  - Noel explicitly included the local case ("even when you need it over time for a local radio"), so do not gate this on the connection being remote.
  - Announce what happened on connect rather than silently flipping a switch: if PC audio comes on because the radio is configured for it, say so. Silent state changes are the thing this whole arc has been correcting.
  - Small, self-contained, and pairs naturally with the roster/per-radio work.

- **THE TRACE/LOG SURFACE — Noel at the radio, 2026-08-11. Four separate problems, and this is the front door of the whole reporting pipeline.** Crash reports, feedback bundles and problem reports all carry this file as their payload, so a confusing entrance breaks the pipeline before any backend exists.
  - **It lives under Help, not Operations.** CLAUDE.md's "Operations → Tracing" is wrong; corrected there.
  - **The automatic trace has no UI control at all.** Tracing runs by default — Noel's point is that Don never turns it on, it is simply always going — but `BootTrace` is a `Friend Boolean` in `globals.vb:115` (with rotation logic around 253+), a code-level flag with no setting behind it. **So the thing that is actually running is the thing you cannot configure**, while the dialog only offers a manual start/stop. Wanted: turn auto-trace on and off, and **choose its level independently** of the manual one.
  - **The button says "Start or Stop Tracing" regardless of state.** That is an accessibility defect, not a wording nit: a toggle that never announces which state it is in tells a screen-reader user nothing. Same family as the no-silent-keystrokes rule. It should say what it will do, or say what is currently happening.
  - **Levels are unexplained.** Nothing tells the operator what a level means or which to pick. Either explain them in plain language or pick a sensible default and remove the choice.
  - **NAMING — Noel: "Jim called these traces, they're really logs aren't they?" He is technically right, but "log" is the one word we cannot use.** They are logs by any normal definition: continuously running, level-filtered, read after the fact to diagnose. Jim's naming came from .NET's `TraceLevel` enum, which is used for ordinary logging all the time. **But in amateur radio "log" means the QSO logbook, and JJFlex already has a Logging feature and a Logging key scope** — so renaming trace to log collides head-on with the strongest existing meaning of the word for our users. **Recommendation: "diagnostic log" (or just "diagnostics").** It is honest about what the thing is, it cannot be confused with contacts, and it already matches the Sprint 29 **diagnostics** tab where the on-demand checks live. Rename the user-facing surface; leave `Tracing.TraceLine` and the internal API alone.
  - Pairs with the **Trace Browser** (Sprint 29 Track H — shipped May 2026, never tested) and the audio-troubleshooting help page. Do this before the R2 report-storage work, not after.

- **NATIVE AUDIO LIBRARIES — researched 2026-08-11, recommendation is update both, separately scoped.** Triggered by Noel asking whether PortAudio and Opus need updating during the audio arc. Full findings below; the short version is that one of them has a real bug in the exact encode path this app uses.
  - **OPUS IS ALREADY 1.6.1 — CLOSED 2026-08-11, no work needed.** Noel recalled an `opus-1.6.1.tar.gz` sitting in `C:\dev`, and probing the shipped DLLs settled it: `opus_get_version_string()` returns **"libopus 1.6.1" on both x64 and x86.** CLAUDE.md's "Opus 1.5.2" was simply stale and has been corrected. **The research recommendation below was therefore acting on wrong documentation — the fixes it identifies are already in the shipped binary.** Retained because the analysis of *which* fixes matter is still the record of why 1.6.1 is the right version to be on, and because the way this was caught is the point: the DLL had no version resource, so a stale doc line was the only claim anyone could check. Third instance today of a hand-maintained description drifting from the thing it describes, after the lying meter and `LeaderKeyHelp()`. **The version-reporting task is cheaper than assumed:** both DLLs already embed readable version strings, so `Pa_GetVersionText()` / `opus_get_version_string()` work on the current binaries today — no `.rc` stamping or rebuild needed for a runtime readout, only for the file-properties view.
  - **PortAudio confirmed at "PortAudio V19.7.0-devel, revision unknown"** on both architectures by the same probe — so that half of the CLAUDE.md claim was accurate, and the recommendation below stands. Note "revision unknown": it was built without git revision info, which is exactly the which-commit-are-we-running ambiguity flagged further down.
  - **(Superseded, kept for the reasoning) Opus: update to 1.6.1 (2026-01-14). Believed at research time to be on 1.5.2 (2024-04-12).** The reason to care is specific, not hygienic: **`compute_stereo_width()` had reversed math** (fixed 2026-01-08) plus an integer overflow (2026-01-07), and `tone_detect()` had a stereo integer overflow (2026-01-13). That function runs in the encoder's stereo-analysis pass under `OPUS_APPLICATION_AUDIO` and drives the mid/side versus left/right coding decision — **which is exactly our TX profile** (Application=Audio, stereo, SWB, 10 ms, ~70 kbps, chosen to match SmartSDR). Also relevant at our bitrate: a low-bitrate encode/decode spreading mismatch fix (2024-09-27). These are correctness bugs in the path that carries the operator's voice, found during the arc that exists to make transmit audio honest.
  - **Opus ABI is additive-only** — header diff between the tags shows new functions and ctl codes, no changed signatures, no removed exports, no enum value changes. **`OpusWrapper.cs` / `OpusEncoder.cs` / `OpusDecoder.cs` need zero changes.** Build via vcpkg (its port is already at 1.6.1) or CMake — note `OPUS_BUILD_SHARED_LIBRARY` **defaults OFF**, so it must be passed explicitly or you get a static lib, and the output is `opus.dll` needing a rename to `libopus.dll`. Our own `runtimes/win-x64/native/README.md` already documents this correctly.
  - **PortAudio: update, but only from git `master` or the daily snapshot — there is no new release.** 19.7.0 (2021-03-07) is still the newest tag; `master` declares 19.8 and the open milestone's blockers are all DirectSound/ALSA/CMake, so **the Windows-backend work has already landed and is not waiting on anything.** vcpkg is pinned at 19.7 and therefore useless here. Worth having: WASAPI mono fixes (PR #934, a workaround for the **Realtek** driver claiming to accept mono then failing to initialise; PR #948, mono I/O in the sync read/write path), the WDM-KS buffer-position alignment fix (PR #931 — the math only worked for power-of-two `bytesPerFrame`, so 2-channel 24-bit corrupted the position and glitched), and a WMME hang fix replacing infinite-timeout loops with bounded ones (2024-03-29).
  - **WHAT THE UPGRADE WILL NOT FIX — important, because it is what we most associate with PortAudio.** The **MME 31-character name truncation is a Win32 limit**, not a PortAudio bug (`MAXPNAMELEN` is 32; the comment is unchanged on current master), and **nothing upstream addresses WDM-KS pins surfacing with empty or misleading names** — the only post-19.7.0 WDM-KS commits are internal debug-log formatting. **So our own workarounds stay necessary:** hiding KS pins by default (`396b1514`) and the WASAPI-display policy are not obsoleted by upgrading. Do not expect otherwise.
  - **COUPLING WORTH KNOWING: the WASAPI mono fixes do not reach us today.** `Devices.cs` filters to exactly 2-channel devices (`StereoOnly`, `maxInputChannels == 2`), so mono devices never appear regardless of how well PortAudio handles them. That filter is precisely the **capability-adapter item in Track E** (accept mono and 4-channel, downmix/upmix, never hide a device for channel count — the filter that hid Noel's real internal mic). **Sequence them together:** upgrading first means the adapter work lands on a backend where mono actually works, instead of exposing devices into known-buggy mono paths.
  - **ACTIONABLE WITHOUT ANY UPGRADE: we never set `PaWasapiStreamInfo`.** The wrapper has no WASAPI-specific stream info anywhere, so opt-in flags — notably **`paWinWasapiAutoConvert`** — are unreachable. No upstream fix was found for the 44.1 kHz-configured device refusing a 48 kHz shared-mode open, and auto-convert is the caller-side lever for exactly that class of mismatch. **Try this before assuming the rate-handshake problem needs a new DLL.**
  - **The version-resource gap persists through an upgrade.** Neither library's CMake embeds a Win32 `VERSIONINFO`, so new DLLs will be just as anonymous as the current ones. Fix it at swap time with a small `.rc` per target or an `rcedit` post-build stamp — otherwise we repeat this exact "what do we actually ship?" question next year. Pairs with the P/Invoke of `Pa_GetVersionText()` / `opus_get_version_string()` for the diagnostics tab.
  - **Security: clean.** No advisories against either C library. The two Opus-adjacent CVEs (CVE-2025-65102, CVE-2026-40614) are bugs in **PJSIP's integration code**, not libopus. Noted only because the class — buffer sizing at the codec integration boundary — applies to our own `OpusDecoder.cs`, which does its own fixed-size allocation (`MaxFrameSize * _channelCount`) independent of libopus. Pre-existing, unrelated to which DLL ships, worth a look sometime.
  - **Scope them separately.** Opus is contained: tagged release, additive ABI, narrow diff. PortAudio rebuilds every backend at once (MME, WASAPI, WDM-KS, DirectSound) off unreleased code, so it needs its own verification pass against the device-enumeration and TX-audio matrices. **Pin and record the snapshot date** the way CLAUDE.md pins FlexLib, or "which unreleased commit are we running" becomes unanswerable.
...
- **Slice list is ordered bottom-to-top (Noel, 2026-08-11).** The Slice A–C list reads/navigates bottom-to-top, so you arrow UP to go A→B→C. Should be top-to-bottom so DOWN-arrow walks A→B→C (natural reading order). Make it a **setting (Noel, 2026-08-11): default top-to-bottom (down-arrow A→B→C), togglable to reverse** for anyone who likes bottom-to-top (flexibility principle). Small fix once the control is identified — candidate is `MultiFlexDialog` (the slice-related dialog) or the Home slice display; confirm which list Noel means before touching. Separate from the honest-TX-audio work. Noticed while testing the 2026-08-11 build; the registration prompt being gone (account-aware roster work) was confirmed good in the same pass.
...
- **Mode reachability gap — SAM/NFM/DFM unreachable by keyboard-first routes (Don, 2026-08-10; diagnosed same day, likely his "can't get to SAM" report).** `RigCaps.ModeTable` carries all ten modes and feeds the main Mode control, Slice menu → Mode, and the mode dialog — SAM is genuinely there. But F10/F11 cycling uses the hardcoded `CommonModes` subset (`MainWindow.xaml.cs:3456` — USB, LSB, CW, DIGU, DIGL, AM, FM) and the Alt accelerators (`NativeMenuBar.cs:1237`) cover only those same seven, so the fast keyboard routes can never land on SAM/NFM/DFM and nothing announces that the cycle is a subset. Fix shape (flexibility principle): a Settings list of which modes join the F10/F11 cycle, default = the current seven; document in `keyboard-reference.md` that cycling is a subset and where the full list lives; consider Command Finder entries per mode ("set mode SAM") so Ctrl+/ reaches any mode by name. Confirm with Don which route he used before closing.
  ...
  - **FIELD-CONFIRMED on Noel's Lenovo, 2026-08-10 (709 diag trace, `C:\temp\JJFlexRadioTrace-20260810-163721.txt`): the stereo-only filter hides real microphones.** The laptop's actual internal mic — Intel Smart Sound Microphone Array — enumerates 4-channel and is `[input HIDDEN by stereo-only filter]` in MME, DirectSound, AND WASAPI; the only surviving "Microphone Array" entries are raw WDM-KS kernel pins with empty display names ("Microphone Array 1 ()"). JJFlex's saved config on that machine had selected exactly that KS pin for input and a leftover "Line 3 (Virtual Audio Cable)" for OUTPUT — radio audio into a virtual cable, mic from a kernel pin, neither a human choice. 70 PortAudio entries on that machine (4 VAC cables × Line/Mic/S-PDIF × 4 APIs + Realtek KS pin zoo). This graduates the audio lane from diagnostics to a fix track:
    ...
    (g) **THE HEADLINE: PC-audio TRANSMIT is dead on current track-branch builds — live-reproduced on Noel's laptop 2026-08-10 with correct devices. NOT established as a 4.2.20 vendor regression: Noel confirmed (2026-08-10 evening) nobody has ever verified PC-audio TX in modern JJ Flexible, and both field machines have only ever run post-hole-punch builds. NEW PRIME SUSPECT: JJFlex's own VitaSocket patches from the 2026-08-07 hole-punch work — `625bdbae` "latch UDP sends onto the radio's observed source port" (correct for WAN punching; on LAN the socket also hears discovery broadcasts from other ports — one mis-latch aims all TX audio at a dead port, silently, with RX unaffected) and `1d037d3a` (ICMP resilience, home of the silent send-guard). Both are track-only; main never had them. **NO RELIABLE BASELINE EXISTS (Noel, same evening): Don never deliberately tested PC-audio TX — he operated the hand mic while his radio was in reach. He reports TX monitor working ONCE under JJ Flexible, mic source unknown — ambiguous, because MicInput only switches to PC while the remote-audio engine runs; a connect without PC audio keys the hardware mic and proves nothing about our path. Noel's successful bench transmit of 2026-08-09 was the workshop session (ms-02 has no microphone), so it doesn't witness the PC-mic chain either. The feature may never have worked in the modern app; the break can sit anywhere from the .NET 10 migration to the hole-punch patches. It surfaced now only because Don's radio moved to Tony's and remote became his only mode. **2026-08-11 SOLVED + a feature vein opens. RF confirmed by DAX IQ (spectral proof) and the meter mystery fully diagnosed with a two-source truth table.**
- **ORGANIZED INTO A PLAN 2026-08-11: `docs/planning/vision/honest-tx-audio.md`** (codename splatter-coach-keydown). That file is now the organizing document — what we build, in what order, what is already shipped and what is merely compiled. The entries below stay the source of record for *rationale* and detail; go there for the track decomposition and execution order.
- **DAX IQ loopback confirmed the transmit carries the tone** (not just power): probe on its own pan (freed two of JJFlex's four by `display panafall remove`), full duplex on, rxant ANT1 = txant ANT1 for internal coupling; keying the 700 Hz tone drove the captured IQ from the −53 dBFS floor to a **rock-steady +703 Hz spike 58 dB up**. Independent of every meter/monitor — actual RF. Route chosen over the MON loopback precisely because it touches none of the suspect plumbing (Noel's call).
- **Meter truth table (build 719 logs SC_MIC, CODEC-output, SW-ALC alongside the old feeds; PC-audio tone vs analog-mic voice):** `SC_MIC` responds to BOTH (−10 PC, tracks voice analog); `CODEC output` BOTH; `SW ALC` BOTH; **`COD-/MIC` (= JJFlex's MicData, "MIC output in CODEC" = the ANALOG mic's ADC path) is −120 for PC audio, responds only to analog**; `HWALC` (= JJFlex's "ALC", the external-amp RCA jack) dead for both. **JJFlex was reading the only two source-blind meters.**
- **THE FIX (small, no branching): point "Check microphone" + the MicData feed at `SC_MIC` instead of the COD-/MIC meter.** `SC_MIC` is downstream of mic *selection* so it's universal — no PC-vs-analog logic needed (the discriminator would have been `mic_selection`, not the PC-audio toggle, but a universal meter moots it). Warning must use a **peak-hold / recent-max over its window** — SC_MIC dips to −150 in the gaps between spoken words, so an instantaneous read would false-alarm every breath. **Separate the two ALC meters — do NOT drop HWALC (Noel, 2026-08-11).** JJFlex conflates them: it reads `HWALC` (the external-amplifier ALC feedback on the RCA jack) and mislabels it "ALC." The transmit-drive meter is SW `ALC`. Fix = separate cleanly: SW ALC drives mic coaching / "Check microphone"; **HWALC stays surfaced as _amplifier_ ALC** — older amps (non-4O3A, no digital/network control) legitimately use the RCA ALC line for amp-overdrive protection, so those operators must keep it. Stop mislabeling, don't remove. FlexLib has no dedicated events for SC_MIC/CODEC/ALC — use the existing public `Radio.FindMeterByName(name)` and subscribe to the Meter's `DataReady` (719 does exactly this as a diag; productionize it).
- **PC-side TX audio pipeline — INPUT RESCUE, the job no radio does (Noel, 2026-08-11).** Architecture: a `TxAudioPipeline` mirroring the existing `RxAudioPipeline`, in JJPortaudio's input callback between mic capture and Opus encode (where LUFS metering also lives). **The point is NOT to duplicate radio DSP — it's to fix the INPUT the radio never sees clearly.** Anchor use case: someone remote with only a laptop internal mic (tinny, hissy, fan + room noise) — PC-side noise gate (kills background between words), EQ/correction (un-thins the laptop response), maybe leveling → makes an unusable mic armchair-copy. **A Flex has TX EQ + speech processor but NO noise gate and no way to repair a bad source mic**, so this serves Flex users too, not just future non-Flex (Hamlib/IC-7300/QRP) radios that lack DSP entirely. Two different jobs: radio-native processing shapes a GOOD signal (surface/expose Flex's TX EQ, don't reimplement it); the PC-side pipeline makes a BAD signal good UPSTREAM of it. **Chain order (Noel's audio-pro friend, 2026-08-11) — standard vocal chain, RIGHT order (clean before you shape):** vocal isolation / **RNNoise FIRST** → **expander/gate** → **parametric EQ** → **compression** (last). Rationale: isolate the voice from room noise before EQ/compression so you're not shaping the noise. RNNoise-first is optional per environment ("noisy NYC apartment" case). **Order should be DYNAMICALLY reorderable** (operator/profile can rearrange stages; default = this order). **EQ-stacking coordination (Noel, 2026-08-11):** Flex's native TX EQ is an 8-band GRAPHIC EQ (fixed centers), NOT parametric. If the operator runs OUR PC-side parametric EQ, flatten the radio's onboard TX EQ (zero all bands) so the two don't stack/fight — and restore it when PC-side EQ is turned off. The pipeline manages the radio's EQ state as a function of whether PC-side EQ is active. General principle: when a PC-side stage duplicates a radio-native one, the pipeline neutralizes the radio's copy to avoid double-processing. **KEY UNLOCK (Noel, 2026-08-11): use STOCK RNNoise, NO training needed** — the standard pre-trained RNNoise model handles room noise fine, and JJFlex's RxAudioPipeline ALREADY runs the RNNoise/Xiph `.rnnn` lineage, so the TX denoise stage reuses that infra with the stock model and ships NOW. Custom-trained `.rnnn` models (the "own Clarity VX" goal) become a quality UPGRADE, not a prerequisite — the pipeline is no longer blocked on the ML training project. Concrete PC-side stages: RNNoise denoise (stock), noise gate/expander (Flex has none), parametric EQ, compression, input EQ/mic-correction, **IR/convolution mic-correction** (apply an impulse response to steer a cheap mic toward a good one — mic modeling; Noel 2026-08-11); rest awaits Noel's friend, don't over-spec. **Starter profiles (Noel, 2026-08-11):** ship presets — "laptop mic," "headset," etc. — that get you 90% there in one selection, mirroring the RX noise profiles and JJSmartAudio's get-you-close philosophy. **STRATEGIC FRAMING — built-in, zero-setup Reaper-style chain (Noel, 2026-08-11; corrected):** many hams build TX audio chains with a DAW (Reaper) + VST plugins (gate/EQ/comp) + virtual cables. NOTE (Noel's correction): DAW/VST accessibility is fine these days — OSARA (Jamie Teh) makes Reaper genuinely usable blind — so the pitch is NOT "only accessible option." The value is **integrated and zero-setup**: no second app alongside, no virtual cables, no plugin chain to license/maintain, travels with JJFlex. **UI — progressive disclosure (Noel, 2026-08-11):** hide the individual knobs (gate threshold, EQ bands, etc.) by default — you pick a profile and you're done — behind a "Show advanced controls" / "show all adjustment points" toggle. Consistent with the geek-values readout: simple default, full detail on request. **Neural tier — TRAIN OUR OWN "Clarity VX"-like model (Noel, 2026-08-11, wants help):** own a neural voice-enhancement/denoise chain rather than depend on third-party plugins. Infra already exists and is shared: RxAudioPipeline runs RNN denoise, the CW neural-decode arc is planned, and the ms-02 NVIDIA GPU is the common training host. So conventional DSP (gate/EQ/IR) gets "part of the way" now; the TX voice model is a real ML workstream (training data = clean-speech + noise/reverb mixtures; architecture family = the RNNoise/DeepFilterNet lineage the RX pipeline already uses, growing toward isolation/de-reverb; deploy via the existing `.rnnn` three-tier model-pack path) that shares tooling with CW. Claude to help scope data/architecture/eval. Flexibility ethos: work with what you have.

- **NEW FEATURE VEIN (Noel, 2026-08-11) — mic-drive coaching, an accessibility differentiator (sighted ops set mic gain against the ALC bar; blind ops can't):** (a) **"How's my audio?" query key** → speaks "coming in hot" / "just right" / "turn it up" judged over a peak-hold window (SW ALC primary = compressor drive, SC_MIC for absolute level); optional continuous mode that only speaks on drift. (b) **"Set my mic level" one-button calibration** → operator talks a few seconds, system watches SC_MIC/ALC and inches `MicLevel` to land peaks in the sweet spot, then announces where it set it. **(c) "JJSmartAudio" (Noel's name/framing, 2026-08-11) — smart-auto-then-refine, measured in LUFS (Noel, 2026-08-11):** use **LUFS (ITU-R BS.1770 / EBU R128 — K-weighted, gated)** as the loudness metric, NOT raw RMS/peak. The gate natively drops the silent gaps between words (solves the "SC_MIC dips to −150 in pauses" false-alarm problem with zero custom peak-hold logic), and K-weighting tracks *perceived* loudness. Compute it PC-side on the raw mic float samples already in JJPortaudio's input callback (pre-Opus, pristine signal). Use momentary (400 ms) LUFS for live coaching, short-term (3 s) for the "how's my audio?" query and the auto-set target, integrated-with-gating for the calibration sample. Auto-set: talk a few seconds → measure LUFS → set `MicLevel` so loudness hits target → announce → operator nudges. **Keep radio ALC as a hard guardrail ON TOP of LUFS** — LUFS says "you sound right," ALC says "you're not overdriving the transmitter"; two different failure modes. Coaching mode is the same engine left running, speaking only on drift. **Measurement-point caveat:** PC-side LUFS is clean for the PC-audio path; the analog-at-the-radio path has no PC-side samples, so it falls back to the SC_MIC/ALC meters (dBFS) or a monitor-stream tap — first cut targets PC audio (the remote operators who most need coaching). **(e) Built-in test-tone generator in the Audio Workshop (Noel, 2026-08-11):** inject a tone (adjustable frequency + level) into the TX PC-audio path so an operator can test/calibrate their transmit chain WITHOUT a mic or without talking — and it's how we calibrate the verdict thresholds. Implement as a tone-source mode in JJPortaudio's TX input (generated samples into the Opus TX stream in place of the mic). **Design (Noel, 2026-08-11):** default **440 Hz** (the media-business reference tone, A440), adjustable (1 kHz etc.). **Replace the mic, don't mix — MUTE the real input while the tone runs** so only the clean tone transmits, no room bleed; insert at the input stage ahead of the pipeline. **Configurable monitor:** let the operator choose whether they hear the tone locally while it goes out, or it's silent to them (both legitimate — confirm by ear vs keep quiet). Directly wanted: the ms-02 has no mic, so today testing needs an external tone-into-a-virtual-cable rig; a built-in generator removes that. **(d) Raw-values readout for power users (Noel, 2026-08-11):** friendly coaching is the default, but expose the live numbers — LUFS (momentary + short-term), ALC dBFS, SC_MIC, forward power — as a screen-reader-navigable read-only-text readout (same pattern as the identity/status cards), and give the "how's my audio?" query a verbose tier that speaks the figures ("just right — short-term −16 LUFS, ALC peaking −3"). Flexibility principle: plain-language by default, full detail on tap. Fits the verbosity architecture (Terse verdict / Chatty verdict+numbers) and the Sprint 29 diagnostics tab. All of this works for PC audio AND analog mic (universal meters), serving remote and at-the-radio operators alike. Positive-side twin of the Check-microphone fix, same meter plumbing. Flexibility principle in miniature — app gets you close, operator keeps final say; togglable, conservative default. Pairs with the audio troubleshooting help page. ("JJSmartAudio" is a user-facing feature name, not a rebrand — internals stay JJFlexRadio.)
- **UX refinements from Noel's 2026-08-11 live test of the honest-TX build:** (1) **LIVE verdict during an audio check** — currently the verdict only speaks at unkey, so you must stop the check to hear it; when adjusting mic gain you need it WHILE the check runs, on a button, so you adjust and hear the effect without stop/restart. Output configurable: plain English, dBFS, or both, spoken. (Natural home: extend the existing "Speak transmit status" command to speak the live mic-audio verdict + level from the running SC_MIC peak-hold — no new key, no keyboard audit.) (2) **Audio menu needs an "Audio Workshop" item** — discoverability; Don has to be able to FIND the workshop. (3) **Audio-menu clarity + the missing PC-audio volume (Noel, 2026-08-11, refined).** The radio-output items (headphone/line-out volume, mutes) adjust the RADIO, not PC audio — a PC-audio operator who bumps "Headphone Level" hears no change. RECOMMENDED shape (relabel + group + fill the gap, not pure hide — hiding breaks hybrid/at-radio monitoring): an **"On-radio outputs" group** ("On-radio headphone volume up/down," "On-radio line-out," mutes) AND a **"PC audio" group** (PC output volume = the existing boost/gain for what you hear through the computer + mic level for TX). The PC-output control is the thing currently MISSING from the menu when PC audio is on — the volume you actually want to change. Labels kill the confusion, grouping keeps everything reachable for hybrid setups. Optional on top: a toggle to hide the on-radio group when PC audio is on for less clutter — but labels alone solve the core problem. **Surface it in THREES (Noel, 2026-08-11):** per the "JJ Flexible in threes" pattern, the audio controls go in the Audio MENU (discovery) AND the Home audio EXPANDER/field group (arrow-navigable, in-context) AND a HOTKEY (fast) — keep the menu restructure and the expander in sync. **HOTKEY = a layered volume sub-mode, NOT flat bindings (Noel, 2026-08-11):** avoid four+ new top-level hotkeys (keymap sprawl). Extend the EXISTING leader key — **Ctrl+J (the "JJ key," J for JJ; leader system already exists in KeyCommands, no timeout, Escape cancels, currently a 2-key system for DSP toggles)** — with a **V volume sub-layer**: Ctrl+J → V enters volume mode, then a target letter (H=on-radio headphone, P=PC output, M=mic level, L=on-radio line-out) with Up/Down to adjust, each announcing its value (no-silent-keystrokes), Escape to exit. JAWS/NVDA layered-keystroke muscle memory. Implementation: add a nested sub-mode to the leader dispatch (`DoLeaderCommand`) — a mode-within-a-mode that persists on volume targets/arrows until Escape. **Leader becomes the AUDIO QUICK-ACCESS HUB (Noel, 2026-08-11):** it already holds the RX DSP toggles; ALSO add the TX-processing controls — **speech processor on/off + level, compander on/off + level** — so a frequent user rides them in three keys (Ctrl+J → compander key → done) instead of menu-digging mid-QSO. Coherent framing: Ctrl+J = "adjust how I sound and what I hear." On/off toggles are single keys after the leader (like the existing DSP toggles); levels use Up/Down in their sub-mode. Work out the exact letters in implementation (avoid collisions); these also live in the Audio menu + expander per the threes pattern. The PC-output volume, mic level, and the on-radio outputs all get fields in the audio expander; the coaching verdict is a hotkey (done — on Speak Transmit Status) plus a readout FIELD in the expander (arrow to it, hear your level) plus a menu entry.

- **STILL BEFORE DON:** (1) PC-audio VOICE test (swap tone for speech → demod IQ to WAV → listen for intelligibility; tone-proven, voice not yet); (2) over-the-air/antenna confirmation via an SDR or a contact (bench 8600 has no antenna; Don's does). Diag builds 708-719 on `diag/don-audio-708`.

**2026-08-10 THE REFRAME (Noel's question cracked it): RF ACTUALLY GOES OUT — `MicData` is the WRONG METER and we may have been chasing a phantom for two days.** Noel asked "did we verify RF went out with the 700 Hz tone?" We never had — every prior conclusion read only `MicData` (SC_MIC). Added forward-power + SWR to the TX telemetry (build 718) and keyed: **FwdPwr jumped 0 → ~30 dBm (1 WATT), steady, the instant we keyed the tone**, while MicData stayed pinned at -120 the whole time. Silence control (tone killed): FwdPwr ~17-24 dBm, varying. So RF leaves the finals and **responds to the audio** (tone = 1 W steady, silence = lower/noisy). Combined with the earlier proof that our captured Opus decodes to a clean 700 Hz tone, the strong read is: **PC-audio transmit has been WORKING; the `MicData`/SC_MIC meter simply does not reflect the PC audio source, and the "Check microphone" PTT warning (which triggers on MicData) is a FALSE ALARM** — the exact units bug flagged earlier (`PttSafetyController` compares a dBFS-floor meter against a linear threshold). This recasts Don entirely: his failure was the dead device selection (proven — KS-pin to a dead jack), and the misleading meter made it undiagnosable. **NOT YET 100% PROVEN** — the definitive confirmation is a DAX IQ capture of the transmitted signal demodulated to check for the clean 700 Hz tone (`tools/rigbench/` daxiq_probe + fdx + demod). Blocked tonight ONLY by the 8600's 4-panadapter limit being fully consumed by JJFlex (removing spare slices didn't free pans; needs JJFlex to hold fewer, or run the probe before JJFlex grabs them, or the MON-loopback via JJFlex's own RX stream). **NEXT-SESSION FIRST MOVES:** (1) confirm via DAX IQ or a listen (MON on, capture JJFlex RX audio = monitored TX, decode for 700 Hz); (2) if confirmed, the real work is small and different from the whole hunt: **fix the SC_MIC/"Check microphone" false alarm** (find the meter that actually reflects PC transmit audio, or gate the warning on FwdPwr/ALC), and ship Don the device-picker fixes. (3) `fdx.py` note independently confirms the global-profile-load-at-connect clobbers full_duplex to 0 — the profile load has real side effects; still worth auditing what else it resets. Diag telemetry builds 708-718 on `diag/don-audio-708`.**
...
**RESOLVED TO A CANDIDATE FIX, same night (pending live verification on 711):** the 710 keyed run exonerated the entire client transport — clean sends from the registered port 4991 to radio:4991, radio ack'd the stream `compression=OPUS`, zero drops/exceptions, correct header bytes — while MicData stayed floored. Two follow-up agents then triangulated: (1) decompiled shipping SmartSDR 4.2.18 (authorized, read-only) — BOTH suspected packet anomalies (packet_size word-rounding over-claim, never-advancing timestamps) ship verbatim in the working client, serializer exonerated; the only field-level difference is the **Opus encode profile: SmartSDR TX = 24 kHz stereo, 70 kbps, Complexity 1, Fullband, FEC off, 10 ms frames; ours was 48 kHz at libopus-default ~102 kbps**. (2) AetherSDR source (C++/Qt, working PC-mic TX) independently confirms the same recipe AND the same two non-bugs, and its fix-commit history ("7 simultaneous bugs found by pcap comparison with SmartSDR") shows wrong-profile choices (mono/20 ms/32 kbps) in the no-RF failure set. Root cause: **the radio's embedded remote-audio decoder accepts one encode profile; 48 kHz Opus meters as silence.** Also explains RX working (PC-side libopus decodes any rate). Fix in diag build **4.1.16.711** (commit `296c8ba3`, Dropbox don\, NAS): TX encoder mirrors the shipped profile; RX untouched; plus radio-side packet-shape telemetry. Caveat: if a capture device rejects a 24 kHz open, JJPortaudio silently falls back to the device rate while the encoder stays at 24 k (pitch-shifted, trace line `rate not supported:24000` exposes it). Hole-punch latch exonerated for LAN. **After live verification: port the encode-profile fix + the trace-spam fixes to `track/flexlib-4220`, and re-run the same test over SmartLink with Don (WAN leg + his device selection walk).** Per-second telemetry proved capture (peak 0.35 speaking) and hand-off (100 Opus pkts/sec into `TXRemoteAudioStream.AddTXData`) healthy while the radio's MicData meter stayed at -120 floor; RX Opus on the same session works. SmartSDR + third-party clients TX fine → client-lib-side. Background-agent investigation (2026-08-10 evening, commit `b9b97d98` on `diag/don-audio-708`) HARD-ruled-out: packet serialization (compiled 4.1.5 vs 4.2.20 serializers, byte-identical across 300 payload sizes × 16 counts), the create command (same string found in shipping SmartSDR's binary), LAN endpoint/port binding (4991 both ways, stream ack'd for our handle), vendoring drift (files match the pristine 4.2.20 drop). Three theories remain, all instrumented in diag build **4.1.16.710** (Dropbox don\, NAS): a silent drop-guard in patched `VitaSocket.SendUdp`, per-packet exceptions swallowed by `AddTXData`'s catch (note: our "pkts delivered" counter counts CALLS, not successes), or the radio defaulting the compression-argument-less TX stream to `compression=none` and binning Opus (fix would be one word in `RequestRemoteAudioTXStream`). One ten-second keyed test on 710 discriminates. **Gates any 4.2.x release — remote operators transmit by PC audio, full stop.**
  ...
  - **Three live-testing catches from the 2026-08-10 evening TX sessions (Noel, laptop):** (1) **"Check microphone" never fires during held Ctrl+Space PTT** — `AlcTimerTick` (PttSafetyController.cs:417) stops the timer in `PttState.PttHold`, so the TX health monitor only covers the transmit-lock path; a hand-held operator transmitting silence gets no warning. (2) **Ctrl+Space PTT does not key when focus is inside the Home expanders** — **FIXED 2026-08-11 on `audio/track-keys` (Keys Track): the PTT gate in `MainWindow_PreviewKeyDown` widened from `FreqOut.IsKeyboardFocusWithin` to also accept `FieldsPanel.IsKeyboardFocusWithin`, so Ctrl+Space / Shift+Space / Escape-unkey work from the expanders. Compile-verified only — needs a keyed radio pass.** The same track fixed the general defect this exposed: `KeyScope.Global` chords died inside every WPF dialog (and Alt+Shift+S got eaten by the Workshop's Save mnemonic) — Window-class handlers now dispatch Global commands and the Ctrl+J leader from any dialog. (3) **"USB" spoken three times** on mode announcement — pre-existing speech duplication, still present.
