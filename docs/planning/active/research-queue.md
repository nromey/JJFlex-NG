...
- **Audio Workshop plan DELIVERED (2026-08-07, Phil2's window): `docs/planning/active/audio-workshop-plan.md` (mox-parrot-sidetone), committed `9b69ac5e` + live refinements.** Absorbed into **Track G** (one track, phased — not three: all slices edit AudioWorkshopDialog). Phase 1 buildable now (Audio Check session riding PttSafetyController, mode-aware monitor, TX-source annotations, help rewrite); phase 2 (record/play) gated on 8600 verification item 1; loopback is a FUTURE follow-up slice (Track H candidate) gated on Don's answers + verification items 3/8. **8600 verification session (plan section 6, items 1–5) runs next: Noel + Phil2 on the test mule** — results appended to the plan file by Phil2 (his deliverable, single-writer). Don's questions doc: `docs/planning/for-don/2026-08-07-transverter-listening-trick.md`, in his Dropbox.
...
Spec is the living plan file `docs/planning/active/audio-workshop-plan.md` (mox-parrot-sidetone). Phase 1: Start Audio Check session (MOX through `PttSafetyController`'s lock path — every existing safeguard applies; low-power-during-checks default ON via the `_savedRFPower` pattern; two-stage Escape, first press unkeys; safety line speaks frequency + power; remote-DAF advisory), mode-aware monitor section (CW vs SB), TX-source annotations (jack-only controls labeled when TX audio is PC-sourced), audio-workshop.md help rewrite, Command Finder registration (no key binding). Phase 2 (gated on verification item 1): slice record/play wrappers + one-button record-then-listen loop. Loopback NOT in this track.
