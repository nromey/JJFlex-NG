...
- **QUEUED (Noel, 2026-08-06 late): radio output levels in the audio settings — headphone slider, line out slider, set-once not step-repeatedly.** Field driver, same night: Noel plugged headphones into the 8600 itself, heard nothing, and read it as an antenna problem — then turned on PC audio and the signal was right there. Almost certainly `HeadphoneGain` sitting at/near zero, and on a non-M radio (no front panel) **software is the only volume knob that exists** — so this is a control-path gap, not a convenience. What exists today, verified in code: FlexBase already wraps `HeadphoneGain`/`LineoutGain` (0–100, `FlexBase.cs:7332-7357`) and FlexLib exposes the mutes (`HeadphoneMute`/`LineoutMute`, plus `FrontSpeakerMute`); ScreenFields Audio (Ctrl+Shift+U) already has direct-set "Headphone Level" / "Line Out Level" value fields (`ScreenFieldsPanel.xaml.cs:410-416`); the Audio menu has stepped Up/Down items (±5, `NativeMenuBar.cs:616+`); HeadphonesUp/Down + LineoutUp/Down commands exist but ship unbound (`KeyCommands.cs:990-993`). The ask: when the audio settings surface displays these levels it also EDITS them — sliders you set once, instead of pressing level-up over and over. Folds into the Settings → Audio migration (see `radio-audio-settings-research.md` addendum): a "Radio Outputs" group (radio-connected only) with headphone level, line out level, and the mutes, live-apply since audio feedback is immediate. **Mystery RESOLVED by Noel same night (third pass):** the silent rig jack was neither level nor mute — **a Flex radio's audio outputs, including the physical headphone/lineout jacks, are silent until a client connects.** Powered-on-but-clientless is not a receiving radio; it's a server waiting for a session. Noel, coming back to the hardware after a break, expected TS-2000 behavior (power on = audio at the jacks) — levels were fine at 50/50 all along. Deliverables from this: (1) **help/docs topic** — Noel wants to voice this when he records documentation; natural written home is `docs/help/md/audio-troubleshooting.md` plus a line in the getting-started material aimed at operators migrating from conventional rigs ("radio on but silent? Connect first — a Flex only makes audio for a connected client"); (2) the planned "why is my radio silent" advisory in the audio audit should check CONNECTED state before levels/mutes — it's the first and most likely rung of the ladder. Direction confirmed by Noel after seeing the existing surfaces: all audio settings ALSO appear in the audio settings surface — the multiple-doors principle, ratified. Audit hooks while in there: (a) the Lineout key handlers refuse to run while PC audio is on (`KeyCommands.cs:578,584` gate on `!rig.PCAudio`) — headphone handlers don't, and the outputs are independent, so the gate looks wrong; (b) `LocalAudioMute` (`FlexBase.cs:7321`) gangs all three outputs and is dead code — decide keep-or-kill; (c) consider a "radio outputs are at zero/muted" visibility affordance so silent-rig-headphones never masquerades as an antenna problem again.
- **QUEUED (Noel, 2026-08-06 late): Release All Extra Slices should keep the ACTIVE slice — and the code says it already does, so find the gap.** Noel's report: the button (Audio-in-slice menu, also Shift+Comma) "releases all slices but the first one"; he wants it to keep the currently active slice instead. Code reality check: `FlexBase.ReleaseAllExtraSlices` (`FlexBase.cs:7263-7285`) already keeps `RXVFO` — the current RX slice — moves TX onto it, and the announcement names the kept slice. But the handler doc comment (`FreqOutHandlers.cs:1836`) says "release every slice except the first," matching Noel's lived experience. So either (a) `RXVFO` doesn't track what the user considers the active slice on some path (menu invocation with focus elsewhere? RXVFO reset after slice churn?), or (b) behavior changed at some point and comment/experience date from a keep-slice-0 era. Verify live with 3-4 slices, active on B or C, from both the menu and Shift+Comma; fix whichever of code/comment/announcement is lying. Noel flagged this as an old suggestion from his earlier testing era and will expound when rested — collect his fuller intent before redesigning beyond the keep-active fix.
