...
**2026-04-20 — R3 revised: Symptom 3 fix lands inside Sprint 26, not as standalone 4.1.17 release.** Noel clarified later the same day that 4.1.17 is reserved for the post-Sprint-27 networking overhaul release — the sprint/release cadence is "work through Sprint 26 + Sprint 27 phases, then ship 4.1.17 as one release." The Symptom 3 fix code already committed on `fix/multiflex-client-list-autorefresh` (commit `589b23bf`) was cherry-picked onto `sprint26/connection-fix` (commit `fe623797`); the version-bump commit (`cafc1a35`) was discarded; the standalone fix branch was deleted. `JJFlexRadio.vbproj` stays at `4.1.16` on the sprint branch and will bump to `4.1.17` when Sprint 26 + Sprint 27 merge to main together. No tester loses access — the fix code is in the sprint branch's debug builds from this point forward. Net architectural effect identical to R3 (same code, same behavior); release mechanics changed.

**2026-04-20 — R4: Phase 4 scope narrowed to truly-dead code; GUIClient-race band-aids stay.** Sprint 26 Phase 0.2 enumerated 13 Sprint-25-era retry sites for Phase 4 deletion, and Phase 0.2's own framing insight correctly anticipated the issue: "some of these band-aids may transfer their *problem* (GUIClient race detection) to the new architecture even if the *mechanism* (ad-hoc retry) is deleted." Phase 4 confirmed this on inspection. The early-abort + grace-period code inside `FlexBase.start()` (`earlyAbortMs`, `removalGraceMs`, `_clientRemovedDuringStart`, `_clientAddedDuringStart`) handles a real and ongoing FlexLib quirk — SmartLink removes and re-adds the GUIClient during setup, and the early-abort path trades a 45-second timeout for a 1-to-15-second retry in the known-bad case. Deleting it would regress user-visible connect times on any transient GUIClient wobble. The `RetryConnect()` method and its call sites in `globals.vb` are the same story: a legitimate retry path for GUIClient-lifecycle failures, not a session-reconnect band-aid. Phase 4 deleted only what was truly dead: the `wan` field, the null-guarded cleanup paths at lines 965-969 / 1770-1773 / 6803, `PreserveWanForRetry()` / `RestoreWanFromRetry()` (zero callers). The GUIClient-race code stays with clearer comments. Living-truth outcome: Phase 4 exits with "exactly one session-level retry mechanism (WanSessionOwner monitor thread)" per the original exit criterion — the remaining FlexBase retry code is radio-level / GUIClient-lifecycle, not session-level.
