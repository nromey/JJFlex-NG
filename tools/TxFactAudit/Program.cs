...
namespace JJFlex.TxFactAudit
{
    ...
    internal static class Program
    {
        ...
        private static int Runbook()
        {
            ...
            Console.WriteLine("  FIRST, before keying, turn ON 'Record the meter stream' on Settings >");
            Console.WriteLine("  Diagnostics. Since task #170 (2026-08-21) the per-meter lines — SWR, the");
            Console.WriteLine("  codec MIC meter, HWALC, forward and reflected power — are behind that switch");
            Console.WriteLine("  and NO trace level brings them back; a Detailed capture alone gets you none of");
            Console.WriteLine("  them. They arrive coalesced, one line per meter per second with min, max and");
            Console.WriteLine("  last, so transient peaks inside each second are already in the max. Without");
            Console.WriteLine("  the switch the trace carries only the correlated txMeters line — SC_MIC with");
            Console.WriteLine("  its peak, SW ALC, and forward power in dBm — which it writes at every level.");
            Console.WriteLine("  A detailed capture (Ctrl+J, Ctrl+D) is still worth starting for everything");
            Console.WriteLine("  else it records; it is just no longer what produces the meter lines.");
            Console.WriteLine("  Then key at one watt with a tone for about five seconds, and unkey.");
            Console.WriteLine("  Then stop the capture, and turn the meter stream back off.");
            Console.WriteLine();
