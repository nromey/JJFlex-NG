            ...
            var stages = new List<FixerStage>
            {
                new FixerStage
                {
                    ...
                    Question = "What is your audio doing right now, and is sound reaching you "
                             + "from the radio?",
                    Explanation =
                        "This stage runs two separate tests, and they answer different "
                        + "questions. The first test reads the open audio stream directly "
                        + "and reports its host API, its input and output device, its "
                        + "sample rate and its channel count. Those readings come from the "
                        + "stream itself rather than from your saved settings, and the two "
                        + "disagree more often than anyone expects. When they disagree, the "
                        + "disagreement is usually the fault, and it is one you would never "
                        + "spot on a settings page. "
                        + "The second test walks the receive chain. It reports the slice "
                        + "you are listening to, the radio's outputs and their levels, how "
                        + "the audio reaches you, and how much of it has actually been "
